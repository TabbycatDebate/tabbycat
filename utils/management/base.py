...
class RoundCommand(TournamentCommand):
    ...
    def add_arguments(self, parser):
        ...
        if self.confirm_round_destruction:
            ...
            rounds_group.add_argument("--all-rounds", action="store_true",
                    help="Run on all rounds in each specified tournament.")
            rounds_group.add_argument("--prelim-rounds", action="store_true",
                    help="Run on all prelim rounds in each specified tournament.")
            rounds_group.add_argument("--break-rounds", action="store_true",
    ...
    def _confirm_rounds(self, rounds, **options):
        if not options["confirm"]:
            ...
            for r in rounds:
                self.stdout.write(self.style.WARNING("  [{t}]: {r}".format(t=r.tournament.name, r=r.name)))
    ...
    def handle_round(self, round, **options):
        ...
        instance corresponding to the slug on the command line."""
        raise NotImplementedError("subclasses of RoundCommand must provide a handle_round() method.")
