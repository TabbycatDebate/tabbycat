...
export default {
  ...
  methods: {
    sendConflict: function (
      conflict, eventType, conflictType, hoverOrPanel,
      clashOrHistory, issuerType
    ) {
      // Issue a Vue message to activate a given conflict type
      const eventCode = `set-conflicts-for-${eventType}`
      const state = (clashOrHistory === 'histories') ? conflict.ago : true
      if (this.debugMode) {
        this.debugLog(
          eventCode, 1, conflict.id, hoverOrPanel, clashOrHistory,
          eventType, conflictType, state, issuerType
        )
      }

      this.$eventHub.$emit(
        eventCode, conflict.id, hoverOrPanel, clashOrHistory,
        eventType, conflictType, state, issuerType
      )
    },
    unsendConflict: function (
      conflict, eventType, conflictType, hoverOrPanel,
      clashOrHistory, issuerType
    ) {
      // Issue a Vue message to deactivate a given conflict type
      const eventCode = `unset-conflicts-for-${eventType}`
      if (this.debugMode) {
        this.debugLog(
          eventCode, 1, conflict.id, hoverOrPanel, clashOrHistory,
    ...
    resetConflictsFor: function (entityType, entityID, hoverOrPanel) {
      ...
      const eventCode = `reset-conflicts-for-${entityType}-${entityID}`
      if (this.debugMode) {
        this.debugLog(
          eventCode, 1, entityID, hoverOrPanel, 'both',
    ...
    forEachConflict: function (conflictsList, callBack) {
      ...
      _.forEach(conflictsList, (types, clashOrHistory) => {
        // Descending into clash or history
        _.forEach(types, (conflicts, type) => {
    ...
    debugLog: function (
      title, tabLevel, id, hoverOrPanel, clashOrHistory,
      eventType, conflictType, state, issuerType
    ) {
