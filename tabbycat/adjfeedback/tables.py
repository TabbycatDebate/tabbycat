...
class FeedbackTableBuilder(TabbycatTableBuilder):
        ...
        breaking_data = [
            {
                ...
                "sort": adj.breaking,
                "type": "breaking",
                "saveURL": reverse_tournament(
                    "adjfeedback-set-adj-breaking-status", self.tournament
                ),
                "id": adj.pk,
            }
    ...
    def add_score_difference_columns(self, adjudicators, scores):
        diff_header = {
            ...
            "icon": "maximize-2",
            "tooltip": _(
                "The current difference between an adjudicator's base score and current score"
            ),
        }
        diff_data = [
            {
                ...
                "tooltip": _(
                    "The difference between this adjudicator's base score and current score"
                ),
            }
            for adj in adjudicators
        ]
    ...
    def add_score_variance_columns(self, adjudicators):
        diff_header = {
            ...
            "icon": "bar-chart-2",
            "tooltip": _(
                "The standard deviation of this adjudicator's current scores; with larger numbers meaning less consistent feedback scores."
            ),
        }
        diff_data = [
            {
                "text": (
                    "%0.1f" % adj.feedback_variance if adj.feedback_variance is not None else ""
                ),
        ...
        link_cell = [
            {
                ...
                "link": reverse_tournament(
                    "adjfeedback-view-on-adjudicator", self.tournament, kwargs={"pk": adj.pk}
                ),
            }
            for adj in adjudicators
        ]
        ...
        if self._show_record_links:
            ...
            def _record_link(progress):
                if isinstance(progress, FeedbackProgressForTeam):
                    url_name = (
                        "participants-team-record"
                        if self.admin
                        else "participants-public-team-record"
                    )
                    pk = progress.team.pk
                elif isinstance(progress, FeedbackProgressForAdjudicator):
                    url_name = (
                        "participants-adjudicator-record"
                        if self.admin
                        else "participants-public-adjudicator-record"
                    )
