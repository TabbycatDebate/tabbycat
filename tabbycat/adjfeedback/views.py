...
class BaseFeedbackOverview(TournamentMixin, VueTableTemplateView):
    ...
    def get_table(self):
        ...
        table = FeedbackTableBuilder(view=self, sort_key=self.sort_key, sort_order=self.sort_order)
    ...
    def get_table(self):
        ...
        for adj in adjudicators:
            ...
            feedback_data.append(
                {
                    "text": ngettext("%(count)d feedback", "%(count)d feedbacks", count)
                    % {"count": count},
                    "sort": count,
                    "link": reverse_tournament(
                        "adjfeedback-view-on-adjudicator", self.tournament, kwargs={"pk": adj.id}
                    ),
                }
            )
...
class FeedbackBySourceView(AdministratorMixin, TournamentMixin, VueTableTemplateView):
    ...
    def get_tables(self):
        ...
        teams = (
            ...
            .annotate(feedback_count=Count("debateteam__adjudicatorfeedback"))
            .prefetch_related("speaker_set")
        )
        team_table = TabbycatTableBuilder(view=self, title=_("From Teams"), sort_key="team")
            ...
            team_feedback_data.append(
                {
                    ...
                    "sort": count,
                    "link": reverse_tournament(
                        "adjfeedback-view-from-team", tournament, kwargs={"pk": team.id}
                    ),
                }
            )
        team_table.add_column({"key": "feedbacks", "title": _("Feedbacks")}, team_feedback_data)

        adjudicators = tournament.adjudicator_set.all().annotate(
            feedback_count=Count("debateadjudicator__adjudicatorfeedback")
        )
        adj_table = TabbycatTableBuilder(view=self, title=_("From Adjudicators"), sort_key="name")
            ...
            adj_feedback_data.append(
                {
                    ...
                    "sort": count,
                    "link": reverse_tournament(
                        "adjfeedback-view-from-adjudicator", tournament, kwargs={"pk": adj.id}
                    ),
                }
            )
...
class FeedbackMixin(TournamentMixin):
    ...
    def get_feedbacks(self):
        ...
        populate_wins_for_debateteams(
            [f.source_team for f in feedbacks if f.source_team is not None]
        )
            ...
            for question in questions:
                for answer in question.answers:
                    if answer["feedback_id"] == feedback.id:
                        feedback.items.append({"question": question, "answer": answer["answer"]})
    ...
    def get_feedback_queryset(self):
        return AdjudicatorFeedback.objects.filter(
            Q(adjudicator__tournament=self.tournament) | Q(adjudicator__tournament__isnull=True),
        ).select_related(
            "adjudicator",
            ...
            teams_table.add_column(
                ...
                [
                    (
                        ...
                        if team.institution
                        else TabbycatTableBuilder.BLANK_TEXT
                    )
                    for team in tournament.team_set.all()
                ],
            )

        adjs_table = TabbycatTableBuilder(
            view=self, sort_key="adjudicator", title=_("An Adjudicator")
        )
...
class AdminAddFeedbackIndexView(AdministratorMixin, BaseAddFeedbackIndexView):
    ...
    def get_from_adj_link(self, adj):
        return reverse_tournament(
            "adjfeedback-add-from-adjudicator", self.tournament, kwargs={"source_id": adj.id}
        )

    def get_from_team_link(self, team):
        return reverse_tournament(
            "adjfeedback-add-from-team", self.tournament, kwargs={"source_id": team.id}
        )
...
class AssistantAddFeedbackIndexView(AssistantMixin, BaseAddFeedbackIndexView):
    ...
    def get_from_adj_link(self, adj):
        return reverse_tournament(
            "adjfeedback-assistant-add-from-adjudicator",
            self.tournament,
            kwargs={"source_id": adj.id},
        )

    def get_from_team_link(self, team):
        return reverse_tournament(
            "adjfeedback-assistant-add-from-team", self.tournament, kwargs={"source_id": team.id}
        )
...
class PublicAddFeedbackIndexView(PublicTournamentPageMixin, BaseAddFeedbackIndexView):
    ...
    def get_from_adj_link(self, team):
        return reverse_tournament(
            "adjfeedback-public-add-from-adjudicator-pk",
            self.tournament,
            kwargs={"source_id": team.id},
        )

    def get_from_team_link(self, team):
        return reverse_tournament(
            "adjfeedback-public-add-from-team-pk", self.tournament, kwargs={"source_id": team.id}
        )
...
class BaseTabroomAddFeedbackView(TabroomSubmissionFieldsMixin, BaseAddFeedbackView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            _("Feedback from %(source)s on %(target)s added.")
            % {
                "source": self.source_name,
                "target": self.adj_feedback.adjudicator.get_public_name(self.tournament),
            },
        )
        return result
...
class PublicAddFeedbackView(
    PublicSubmissionFieldsMixin, PersonalizablePublicTournamentPageMixin, BaseAddFeedbackView
):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            _("Thanks, %(source)s! Your feedback on %(target)s has been recorded.")
            % {
                "source": self.source_name,
                "target": self.adj_feedback.adjudicator.get_public_name(self.tournament),
            },
    ...
    def get_success_url(self):
        ...
        if isinstance(self.object, Adjudicator):
            return reverse_tournament(
                ...
                self.tournament,
                kwargs={"url_key": self.object.url_key},
            )
        elif isinstance(self.object, Speaker):
            return reverse_tournament(
    ...
    def get_success_url(self):
        ...
        if isinstance(self.object, Adjudicator):
            return reverse_tournament(
                "adjfeedback-public-add-from-adjudicator-pk",
                self.tournament,
                kwargs={"source_id": self.object.id},
            )
        elif isinstance(self.object, Team):
            return reverse_tournament(
                "adjfeedback-public-add-from-team-pk",
                self.tournament,
                kwargs={"source_id": self.object.id},
            )
        else:
...
class BaseAdjudicatorActionView(
    LogActionMixin, AdministratorMixin, TournamentMixin, PostOnlyRedirectView
):
    ...
    def get_adjudicator(self, request):
        try:
            ...
            raise AdjudicatorActionError(
                _("Whoops! I didn't recognise that adjudicator: %(adjudicator)s")
                % {"adjudicator": adj_id}
            )
        return adjudicator
...
class SetAdjudicatorBaseScoreView(BaseAdjudicatorActionView):
    ...
    def modify_adjudicator(self, request, adjudicator):
        ...
        atsh = AdjudicatorBaseScoreHistory(
            adjudicator=adjudicator, round=self.tournament.current_round, score=score
        )
        atsh.save()
        self.atsh = atsh
...
class BaseFeedbackProgressView(TournamentMixin, VueTableTemplateView):
    ...
    def get_tables(self):
        ...
        adjs_table = FeedbackTableBuilder(
            view=self, title=_("From Adjudicators"), sort_key="owed", sort_order="desc"
        )
        ...
        teams_table = FeedbackTableBuilder(
            view=self, title=_("From Teams"), sort_key="owed", sort_order="desc"
        )
...
class BaseFeedbackToggleView(AdministratorMixin, TournamentMixin, PostOnlyRedirectView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(
            self.request,
            _("Feedback for %(adjudicator)s from %(source)s is now %(result)s.")
            % {
...
class AdjudicatorFeedbackCsvView(FeedbackMixin, AdministratorMixin, TournamentMixin, BaseCsvView):
    ...
    def write_rows(self, writer):
        headers = [
            ...
            "adjudicator.name",
            "adjudicator.type",
            "source_adjudicator.id",
            "source_adjudicator.name",
            "source_adjudicator.type",
            "source_team.id",
            "source_team.short_name",
            "source_team.result",
            "score",
            "ignored",
        ]
        ...
        for f in feedbacks:
            row = [
                f.round.seq,
                f.round.abbreviation,
                f.adjudicator.id,
                f.adjudicator.name,
                f.debate_adjudicator.get_type_display(),
            ]
