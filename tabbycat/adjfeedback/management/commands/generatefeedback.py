...
from ...dbutils import (
    add_feedback,
    add_feedback_to_round,
    delete_all_feedback_for_round,
    delete_feedback,
)
...
class Command(RoundCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-u", "--user", type=str, help="Username of submitter", default="random"
        )

        status = parser.add_mutually_exclusive_group()
        status.add_argument(
            "-D", "--discarded", action="store_true", help="Make feedback discarded"
        )
        status.add_argument(
            "-c", "--confirmed", action="store_true", help="Make feedback confirmed"
        )

        parser.add_argument(
            "--clean", help="Remove all associated feedback first", action="store_true"
        )
        parser.add_argument(
            "--create-user", help="Create user if it doesn't exist", action="store_true"
        )
        ...
        for tournament in self.get_tournaments(options):
            for debate_id in options["debates"]:
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            "Warning: There is no debate with "
    ...
    def handle_object(self, obj, name_field, delete_func, add_func, **options):
        if options["clean"]:
            self.stdout.write(
                self.style.WARNING("Deleting all feedback for {}...".format(name_field(obj)))
            )
            delete_func(obj)

        self.stdout.write(
            self.style.MIGRATE_HEADING("Generating feedback for {}...".format(name_field(obj)))
        )
        try:
    ...
    def handle_debate(self, debate, **options):
        return self.handle_object(
            debate, attrgetter("matchup"), delete_feedback, add_feedback, **options
        )
