...
class BaseFeedbackForm(forms.Form):
    ...
    def _make_question_field(self, question):
        if question.answer_type == question.ANSWER_TYPE_BOOLEAN_SELECT:
            ...
            if min_value is None or max_value is None:
                logger.error(
                    "Integer scale %r has no min_value or no max_value" % question.reference
                )
            ...
            field = forms.MultipleChoiceField(
                choices=question.choices_for_field, widget=BlockCheckboxWidget()
            )
    ...
    def _create_fields(self):
        ...
        score_label = mark_safe(
            _("Overall score (%(min)d=worst; %(max)d=best)*")
            % {"min": int(adj_min_score), "max": int(adj_max_score)}
        )
        self.fields["score"] = forms.FloatField(
            min_value=adj_min_score, max_value=adj_max_score, label=score_label
        )
...
def make_feedback_form_class_for_adj(
    source,
    tournament,
    submission_fields,
    confirm_on_submit=False,
    ...
    def adj_choice(adj, debate, pos):
        ...
        display += _("%(name)s (%(adjpos)s)") % {
            "name": adj.get_public_name(tournament),
            "adjpos": ADJUDICATOR_POSITION_NAMES[pos],
        }
        return value, display

    adjfeedback_query = AdjudicatorFeedback.objects.filter(
        source_adjudicator__adjudicator=source,
        source_adjudicator__debate=OuterRef("debate"),
        adjudicator=OuterRef("adjudicator"),
        confirmed=True,
    )
    debateadjs = (
        ...
        .prefetch_related(
            Prefetch(
                ...
                queryset=DebateAdjudicator.objects.all()
                .select_related("adjudicator")
                .annotate(submitted=Exists(adjfeedback_query)),
            ),
        )
    )
    ...
    if include_unreleased_draws:
        debateadjs = debateadjs.filter(
            debate__round__draw_status__in=[Round.Status.CONFIRMED, Round.Status.RELEASED]
        )
    else:
    ...
    class FeedbackForm(BaseFeedbackForm):
        ...
        target = RequiredTypedChoiceField(
            choices=choices,
            coerce=BaseFeedbackForm.coerce_target,
...
def make_feedback_form_class_for_team(
    source,
    tournament,
    submission_fields,
    confirm_on_submit=False,
                ...
                .annotate(
                    submitted=Exists(
                        AdjudicatorFeedback.objects.filter(
                            source_team__team=source,
                            source_team__debate=OuterRef("debate"),
                            adjudicator=OuterRef("adjudicator"),
                            confirmed=True,
                        ),
                    )
    ...
    if include_unreleased_draws:
        debates = debates.filter(
            round__draw_status__in=[Round.Status.CONFIRMED, Round.Status.RELEASED]
        )
    else:
...
class UpdateAdjudicatorScoresForm(forms.Form):
            ...
            try:
                ...
                errors_in_line.append(
                    ImportValidationError(
                        i,
                        _(
                            'There are several adjudicators called "%(adjudicator)s", so '
                ...
                errors_in_line.append(
                    ImportValidationError(
                        i,
                        _(
                            "There is no adjudicator in this tournament with the "
                            'name "%(adjudicator)s"'
                        )
                        % {"adjudicator": name},
                    )
    ...
    def save(self):
        ...
        logger.info(
            "UpdateAdjudicatorScoresForm: Saving base score histories to database done (5 of 5)"
        )
