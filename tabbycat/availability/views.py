...
class AvailabilityIndexView(RoundMixin, AdministratorMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["availability_info"] = OrderedDict(
            [("teams", teams), ("adjs", adjs), ("venues", venues)]
        )
    ...
    def _get_breaking_teams_dict(self):
        ...
        if self.round.prev is None or not self.round.prev.is_break_round:
            ...
            nadvancing += self.round.prev.debate_set.all().aggregate(
                lowest_room=Coalesce(Min("room_rank") - 1, 0)
            )["lowest_room"]
    ...
    def get_table(self):
        ...
        if self.round.prev:
            title = _("Active in %(prev_round)s") % {
                "prev_round": escape(self.round.prev.abbreviation)
            }
            table.add_column(
                {"key": "active-prev", "title": title},
                [
                    {
...
class AvailabilityTypeTeamView(AvailabilityTypeBase):
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related("speaker_set__checkin_identifier")
        )
...
class CheckInAllBreakingAdjudicatorsView(BaseBulkActivationView):
    ...
    def activate_function(self):
        utils.set_availability(
            self.tournament.relevant_adjudicators.filter(breaking=True), self.round
        )


class CheckInAllFromPreviousRoundView(BaseBulkActivationView):
    activation_msg = gettext_lazy(
        "Checked in all teams, adjudicators and rooms from previous round."
    )
    ...
    def activate_function(self):
        ...
        items = [
            (Team, t.team_set),
            (Adjudicator, t.relevant_adjudicators),
            (Venue, t.relevant_venues),
        ]
        ...
        for model, relevant_instances in items:
            ...
            previous_ids = set(
                a["object_id"]
                for a in RoundAvailability.objects.filter(
                    content_type=contenttype, round=self.round.prev
                ).values("object_id")
            )
            ...
            logger.debug(
                "Checking in %s: %s", model._meta.verbose_name_plural, previous_relevant_ids
            )
