...
def annotate_availability(queryset, round):
    ...
    if round.prev:
        queryset = queryset.prefetch_related(
            Prefetch(
                "round_availabilities",
                queryset=RoundAvailability.objects.filter(round=round.prev),
                to_attr="prev_availability",
            )
        )
...
def set_availability_by_id(model, ids, round):
    ...
    if model not in [Adjudicator, Team, Venue]:
        logger.error(
            "Bad model in set_availability_by_id: %s", model.__class__.__name__, stack_info=True
        )
        return  # do nothing
    ...
    existing = set(
        a["object_id"]
        for a in RoundAvailability.objects.filter(content_type=contenttype, round=round).values(
            "object_id"
        )
    )
