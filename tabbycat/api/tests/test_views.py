...
class MotionViewsetTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def test_no_public_access_preferences(self):
        ...
        response = self.client.get(
            reverse("api-motion-list", kwargs={"tournament_slug": self.tournament.slug})
        )
        self.assertEqual(response.status_code, 401)
    ...
    def test_unauthorized_motion_detail(self):
        ...
        response = self.client.get(
            reverse("api-motion-detail", kwargs={"tournament_slug": self.tournament.slug, "pk": 1})
        )
...
class SpeakerCategoryViewsetTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def test_private_excluded_public(self):
        response = self.client.get(
            reverse("api-speakercategory-list", kwargs={"tournament_slug": self.tournament.slug})
        )
...
class BreakEligibilityViewsetTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def test_get_eligible_teams(self):
        ...
        response = self.client.get(
            reverse(
                "api-breakcategory-eligibility",
                kwargs={"tournament_slug": self.tournament.slug, "pk": 1},
            )
        )
        self.assertEqual(len(response.data), 2)
...
class SpeakerEligibilityViewsetTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def setUp(self):
        super().setUp()
        self.sc = self.tournament.speakercategory_set.create(
            name="sc1", slug="sc1", seq=1, public=False
        )
    ...
    def test_get_eligible_speakers(self):
        ...
        response = self.client.get(
            reverse(
                "api-speakercategory-eligibility",
                kwargs={"tournament_slug": self.tournament.slug, "pk": self.sc.pk},
            )
        )
        self.assertEqual(len(response.data), 2)
...
class BreakingTeamsViewsetTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def test_get_breaking_teams(self):
        ...
        response = self.client.get(
            reverse(
                "api-breakcategory-break", kwargs={"tournament_slug": self.tournament.slug, "pk": 1}
            )
        )
