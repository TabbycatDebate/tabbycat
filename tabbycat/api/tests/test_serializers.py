...
class RoundSerializerTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def test_include_motions_if_released(self):
        ...
        response = self.client.get(
            reverse_round("api-round-detail", self.tournament.round_set.first())
        )
    ...
    def test_seq_validation(self):
        ...
        response = client.post(
            reverse_tournament("api-round-list", self.tournament),
            {
                "motions": [],
                "seq": 1,
                "name": "Round 1",
    ...
    def test_break_category_validation(self):
        ...
        response = client.patch(
            reverse_round("api-round-detail", round),
            {
                "break_category": None,
            },
        )
    ...
    def test_can_give_start_datetime(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            datetime.fromisoformat(response.data["starts_at"]),
            datetime(2023, 11, 18, 11, 0, 0, tzinfo=tz),
        )
    ...
    def test_can_give_start_time(self):
        ...
        self.assertEqual(
            datetime.fromisoformat(response.data["starts_at"]),
            datetime.combine(date.today(), time(0, 0, 0, tzinfo=tz)),
        )
...
class MotionSerializerTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def test_create_motion_with_round(self):
        ...
        client.login(username="admin", password="admin")
        response = client.post(
            reverse_tournament("api-motion-list", self.tournament),
            {
                "text": "This House would straighten all bananas",
                "reference": "Bananas",
                "info_slide": "Get bent",
                "rounds": [
                    {"seq": 4, "round": "http://testserver/api/v1/tournaments/demo/rounds/1"}
                ],
            },
        )
        self.assertEqual(response.status_code, 201)
...
class AdjudicatorSerializerTests(CompletedTournamentTestMixin, APITestCase):
    ...
    def test_create_adj_null_institution(self):
        ...
        client.login(username="admin", password="admin")
        response = client.post(
            reverse_tournament("api-adjudicator-list", self.tournament),
            {
                "name": "string",
                "gender": "M",
...
class BallotSerializerTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_superuser(
            username="admin1", password="admin", is_active=True
        )
        ...
        DebateTeam.objects.bulk_create(
            [
                DebateTeam(side=side, team=team, debate=self.debate)
                for side, team in zip(["aff", "neg"], [self.t1, self.t2])
            ]
        )
        DebateAdjudicator.objects.bulk_create(
            [
                DebateAdjudicator(adjudicator=self.a1, debate=self.debate, type="C"),
                DebateAdjudicator(adjudicator=self.a2, debate=self.debate, type="P"),
            ]
        )
        ...
        RoundMotion.objects.bulk_create(
            [
                RoundMotion(seq=i, motion=motion, round=self.round)
                for i, motion in enumerate([self.m1, self.m2])
            ]
        )
    ...
    def test_can_create_consensus_ballot_scores(self):
        ...
        response = client.post(
            reverse_round("api-ballot-list", self.round, kwargs={"debate_pk": self.debate.pk}),
            {
                "result": {
                    "sheets": [
                        {
                                    ...
                                    "speeches": [
                                        {
                                            "ghost": False,
                                            "score": 80,
                                            "speaker": reverse_tournament(
                                                "api-speaker-detail",
                                                self.tournament,
                                                kwargs={"pk": self.s1.pk},
                                            ),
                                        },
                                        {
                                            "ghost": False,
        ...
        response = client.post(
            reverse_round("api-ballot-list", self.round, kwargs={"debate_pk": self.debate.pk}),
            {
                "result": {
                    "sheets": [
                        {
                            "adjudicator": reverse_tournament(
                                "api-adjudicator-detail", self.tournament, kwargs={"pk": self.a1.pk}
                            ),
                            "teams": [
                                {
                                    "side": "aff",
                        ...
                        {
                            "adjudicator": reverse_tournament(
                                "api-adjudicator-detail", self.tournament, kwargs={"pk": self.a2.pk}
                            ),
                            "teams": [
                                {
                                    "side": "aff",
        ...
        response = client.post(
            reverse_round("api-ballot-list", self.round, kwargs={"debate_pk": self.debate.pk}),
            {
                ...
                "vetos": [
                    {
                        "team": reverse_tournament(
                            "api-team-detail", self.tournament, kwargs={"pk": self.t1.pk}
                        ),
                        "motion": reverse_tournament(
                            "api-motion-detail", self.tournament, kwargs={"pk": self.m1.pk}
                        ),
                    },
                    {
                        "team": reverse_tournament(
                            "api-team-detail", self.tournament, kwargs={"pk": self.t2.pk}
                        ),
                        "motion": reverse_tournament(
                            "api-motion-detail", self.tournament, kwargs={"pk": self.m2.pk}
                        ),
                    },
                ],
            },
        ...
        response = client.post(
            reverse_round("api-ballot-list", self.round, kwargs={"debate_pk": self.debate.pk}),
            {
                ...
                "vetos": [
                    {
                        "team": reverse_tournament(
                            "api-team-detail", self.tournament, kwargs={"pk": self.t3.pk}
                        ),
                        "motion": reverse_tournament(
                            "api-motion-detail", self.tournament, kwargs={"pk": self.m1.pk}
                        ),
    ...
    def test_adj_not_in_debate(self):
            ...
            reverse_round("api-ballot-list", self.round, kwargs={"debate_pk": self.debate.pk}),
            {
                "result": {
                    "sheets": [
                        {
                            "adjudicator": reverse_tournament(
                                "api-adjudicator-detail", self.tournament, kwargs={"pk": self.a3.pk}
                            ),
                            "teams": [
                                {
                                    "side": "aff",
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            str(response.data["result"]["sheets"][0]["adjudicator"][0]),
            "Adjudicator must be in debate",
        )
    ...
    def test_single_adj_many_ballots_fail(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            str(response.data["single_adj"]),
            "Single-adjudicator ballots can only have one scoresheet",
        )
    ...
    def test_incomplete_ballots_fail(self):
        ...
        DebateAdjudicator.objects.create(
            adjudicator=self.a3, type=DebateAdjudicator.TYPE_PANEL, debate=self.debate
        )
        ...
        self.assertEqual(
            ...
            "Voting ballots must either have one scoresheet or ballots from all voting adjudicators",
        )
    ...
    def test_many_ballots_consensus_fail(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            str(response.data["result"]), "Consensus ballots can only have one scoresheet"
        )
    ...
    def test_inconsistent_speaker_order(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            str(response.data["result"]["non_field_errors"][0]), "Inconsistant speaker order"
        )
    ...
    def test_speaks_without_scores(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            str(response.data["result"]["sheets"][0]["teams"][0]["non_field_errors"][0]),
