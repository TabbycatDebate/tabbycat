...
from .mixins import (
    AdministratorAPIMixin,
    APILogActionMixin,
    PublicAPIMixin,
    RoundAPIMixin,
    TournamentAPIMixin,
    TournamentPublicAPIMixin,
)
...
tournament_parameter = OpenApiParameter(
    "tournament_slug", description="The tournament's slug", type=str, location="path"
)
round_parameters = [
    tournament_parameter,
    OpenApiParameter(
        "round_seq", description="The round's sequence number", type=int, location="path"
    ),
]
debate_parameters = [
    *round_parameters,
    OpenApiParameter(
        "debate_pk", description="The debate's primary key", type=int, location="path"
    ),
]
id_parameter = OpenApiParameter(
    "id", description="The object's primary key", type=int, location="path"
)
...
class APIV1RootView(PublicAPIMixin, GenericAPIView):
    ...
    def get(self, request, format=None):
        ...
        institution_create_url = reverse(
            "api-global-institution-list", request=request, format=format
        )
...
class TournamentViewSet(PublicAPIMixin, APILogActionMixin, ModelViewSet):
    ...
    queryset = Tournament.objects.all().prefetch_related(
        ...
        Prefetch(
            "round_set",
            queryset=Round.objects.filter(completed=False)
            .annotate(Count("debate"))
            .order_by("seq"),
...
class TournamentPreferenceViewSet(
    TournamentFromUrlMixin, AdministratorAPIMixin, APILogActionMixin, PerInstancePreferenceViewSet
):
    """ """
...
class RoundViewSet(TournamentAPIMixin, PublicAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related(
                "roundmotion_set", "roundmotion_set__motion", "roundmotion_set__motion__tournament"
            )
        )
...
class MotionViewSet(TournamentAPIMixin, TournamentPublicAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        filters = Q()
        if self.tournament.pref("public_motions") and not (
            self.tournament.pref("motion_tab_released") or self.request.user.is_staff
        ):
        ...
        return (
            ...
            .get_queryset()
            .filter(filters)
            .prefetch_related("roundmotion_set", "roundmotion_set__round")
        )
...
class BreakingTeamsView(
    TournamentAPIMixin, TournamentPublicAPIMixin, GenerateBreakMixin, GenericViewSet, ListModelMixin
):
    ...
    def break_category(self):
        if not hasattr(self, "_break_category"):
            self._break_category = get_object_or_404(
                BreakCategory, tournament=self.tournament, pk=self.kwargs.get("pk")
            )
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .filter(break_category=self.break_category)
    ...
    def update(self, request, *args, **kwargs):
        serializer = serializers.PartialBreakingTeamSerializer(
...
class TeamViewSet(TournamentAPIMixin, TournamentPublicAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        category_prefetch = Prefetch(
            "categories", queryset=SpeakerCategory.objects.all().select_related("tournament")
        )
        if not self.request.user or not self.request.user.is_staff:
        ...
        return (
            ...
            .prefetch_related(
                Prefetch(
                    ...
                    .select_related("team__tournament", "checkin_identifier"),
                ),
...
class AdjudicatorViewSet(TournamentAPIMixin, TournamentPublicAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            super()
            .get_queryset()
            .select_related("checkin_identifier")
            .prefetch_related(
                "team_conflicts",
                "team_conflicts__tournament",
                "adjudicator_conflicts",
                "adjudicator_conflicts__tournament",
                "institution_conflicts",
                "venue_constraints__category__tournament",
            )
            .filter(filters)
...
class GlobalInstitutionViewSet(AdministratorAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get("region"):
            filters &= Q(region__name=self.request.query_params["region"])
        return (
            Institution.objects.filter(filters)
            .select_related("region")
            .prefetch_related("venue_constraints__category__tournament")
        )
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .prefetch_related(
                Prefetch(
                    "venuecategory_set",
                    queryset=VenueCategory.objects.select_related("tournament").filter(
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .prefetch_related(
                Prefetch(
                    "venues",
                    queryset=Venue.objects.select_related("tournament").filter(
                        tournament__isnull=False
                    ),
                )
            )
...
class BaseCheckinsView(AdministratorAPIMixin, TournamentAPIMixin, APIView):
    ...
    def broadcast_checkin(self, obj, check):
        ...
        checkin = None
        if check:
            checkin = Event.objects.create(
                identifier=obj.checkin_identifier, tournament=self.tournament
            )
    ...
    def get_response_dict(self, request, obj, checked, event, **kwargs):
        return {
            ...
            "checked": checked,
            "timestamp": (
                DateTimeField().to_representation(event.time) if event is not None else None
            ),
        }

    def get_queryset(self):
        return self.model.objects.filter(**self.lookup_kwargs()).select_related(
            self.tournament_field
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        event = get_unexpired_checkins(self.tournament, self.window_preference_pref).filter(
    ...
    def patch(self, request, *args, **kwargs):
        ...
        obj = self.get_object()
        events = get_unexpired_checkins(self.tournament, self.window_preference_pref).filter(
            identifier=obj.checkin_identifier
        )
        check = events.exists()
...
class BaseStandingsView(TournamentAPIMixin, TournamentPublicAPIMixin, GenericAPIView):
    ...
    def get_metrics(self):
        if self.request.query_params.get("metrics"):
            return self.request.query_params.get("metrics").split(
                ","
            ), self.request.query_params.get("extra_metrics").split(",")

        pref_model = self.model.__name__.lower()
        return self.tournament.pref(pref_model + "_standings_precedence"), self.tournament.pref(
            pref_model + "_standings_extra_metrics"
        )

    def get_queryset(self):
        qs = self.model.objects.filter(**{self.tournament_field: self.tournament}).select_related(
            self.tournament_field
        )
        return qs
    ...
    def get_max_round(self):
        if self.request.query_params.get("round"):
            return Round.objects.get(
                tournament=self.tournament, seq=int(self.request.query_params.get("round"))
            )
        return None
...
@extend_schema_view(
    list=extend_schema(
        ...
        responses=serializers.SpeakerRoundScoresSerializer(many=True),
    ),
)
class SpeakerRoundStandingsRoundsView(TournamentAPIMixin, TournamentPublicAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        qs = (
            ...
            .prefetch_related(
                Prefetch(
                    "team__debateteam_set",
                    queryset=DebateTeam.objects.all().select_related("debate__round__tournament"),
                )
            )
        )
        ...
        speaker_scores = (
            SpeakerScore.objects.select_related(
                "speaker", "ballot_submission", "debate_team__debate__round__tournament"
            )
            .filter(
                ballot_submission__confirmed=True,
                speaker_id__in=data.keys(),
            )
        ...
        if self.request.query_params.get("replies", False) == "true":
            ...
            speaker_scores = speaker_scores.filter(
                position__lte=self.tournament.last_substantive_position
            )
...
@extend_schema_view(
    list=extend_schema(
        ...
        responses=serializers.TeamRoundScoresSerializer(many=True),
    ),
)
    ...
    def get_queryset(self):
        ...
        qs = (
            ...
            .prefetch_related(
                Prefetch(
                    "debateteam_set",
                    queryset=DebateTeam.objects.all()
                    .prefetch_related(ts_pf)
                    .select_related("debate__round__tournament"),
                )
            )
        )
...
class PairingViewSet(RoundAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .select_related("round", "round__tournament", "venue", "venue__tournament")
            .prefetch_related(
                "debateteam_set",
                ...
                "debateadjudicator_set__adjudicator__tournament",
            )
        )
...
@extend_schema_view(
    ...
    update=extend_schema(
        ...
        request=serializers.UpdateBallotSerializer,
    ),
...
class BallotViewSet(RoundAPIMixin, TournamentPublicAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related(
                "motion", "motion__tournament", "participant_submitter__adjudicator__tournament"
            )
        )
    ...
    @extend_schema(
        ...
        parameters=[id_parameter],
        responses={200: serializers.BallotSerializer},
    )
    def destroy(self, request, *args, **kwargs):
    ...
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                ...
                required=False,
                type=str,
                enum=["adjudicator", "team"],
            ),
            OpenApiParameter(
                "source",
                description="The ID of the participant submitting feedback; must be used in conjunction with `source_type`",
                required=False,
                type=int,
            ),
            OpenApiParameter(
...
class FeedbackViewSet(TournamentAPIMixin, AdministratorAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if query_params.get("round"):
            filters &= Q(
                source_adjudicator__debate__round__seq__in=query_params.get("round").split(",")
        ...
        return (
            ...
            .select_related(
                "adjudicator",
                "adjudicator__tournament",
                "source_adjudicator",
                "source_team",
                "source_team__team",
                "source_adjudicator__adjudicator__tournament",
                "source_team__team__tournament",
                "source_adjudicator__debate",
                "source_team__debate",
                ...
                "participant_submitter__speaker__team__tournament",
            )
...
class AvailabilitiesViewSet(RoundAPIMixin, AdministratorAPIMixin, APIView):
    ...
    extra_params = [
        ...
        OpenApiParameter(
            "teams", description="Only include teams", required=False, type=bool, default=False
        ),
        OpenApiParameter(
            "venues", description="Only include rooms", required=False, type=bool, default=False
        ),
    ]

    def get_field(self):
        field = ParticipantAvailabilityForeignKeyField(
            many=True, view_name="api-availability-list"
        )  # Dummy view name
    ...
    def get_queryset(self):
        return RoundAvailability.objects.filter(
            ~self.get_filters(), round=self.round
        ).select_related("content_type", "round__tournament")
    ...
    def patch(self, request, *args, **kwargs):
        objs = sorted(
            self.get_field().to_internal_value(request.data), key=lambda o: type(o).__name__
        )
        for model, participants in groupby(objs, key=type):
            ...
            RoundAvailability.objects.bulk_create(
                [
                    RoundAvailability(content_type=contenttype, round=self.round, object_id=id)
    ...
    def put(self, request, *args, **kwargs):
        ...
        for model, participants in groupby(objs, key=type):
            ...
            RoundAvailability.objects.bulk_create(
                [
                    RoundAvailability(content_type=contenttype, round=self.round, object_id=p.id)
...
class PreformedPanelViewSet(RoundAPIMixin, AdministratorAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related(
                ...
                "preformedpaneladjudicator_set__adjudicator",
    ...
    def add_blank(self, request, *args, **kwargs):
        ...
        for i, (bracket_min, bracket_max, liveness) in enumerate(
            calculate_anticipated_draw(self.round), start=1
        ):
            PreformedPanel.objects.update_or_create(
                round=self.round,
                room_rank=i,
                defaults={
...
class UserViewSet(AdministratorAPIMixin, ModelViewSet):
    ...
    def get_queryset(self):
        qs = get_user_model().objects.prefetch_related(
            "membership_set__group__tournament", "userpermission_set__tournament"
        )
        for user in qs:
