...
from adjfeedback.models import (
...
def _validate_field(self, field, value):
    ...
    qs = self.Meta.model.objects.filter(
        tournament=self.context["tournament"], **{field: value}
...
class VenueConstraintSerializer(serializers.ModelSerializer):
    category = fields.TournamentHyperlinkedRelatedField(
        view_name="api-venuecategory-detail", queryset=VenueCategory.objects.all()
    )
...
class TournamentSerializer(serializers.ModelSerializer):
    ...
    url = serializers.HyperlinkedIdentityField(
        view_name="api-tournament-detail", lookup_field="slug", lookup_url_kwarg="tournament_slug"
    )
    ...
    class TournamentLinksSerializer(serializers.Serializer):
        rounds = serializers.HyperlinkedIdentityField(
            view_name="api-round-list", lookup_field="slug", lookup_url_kwarg="tournament_slug"
        )
        ...
        teams = serializers.HyperlinkedIdentityField(
            view_name="api-team-list", lookup_field="slug", lookup_url_kwarg="tournament_slug"
        )
        ...
        speakers = serializers.HyperlinkedIdentityField(
            view_name="api-speaker-list", lookup_field="slug", lookup_url_kwarg="tournament_slug"
        )
        venues = serializers.HyperlinkedIdentityField(
            view_name="api-venue-list", lookup_field="slug", lookup_url_kwarg="tournament_slug"
        )
        ...
        motions = serializers.HyperlinkedIdentityField(
            view_name="api-motion-list", lookup_field="slug", lookup_url_kwarg="tournament_slug"
        )
        feedback = serializers.HyperlinkedIdentityField(
            view_name="api-feedback-list", lookup_field="slug", lookup_url_kwarg="tournament_slug"
        )
...
class RoundSerializer(serializers.ModelSerializer):
    class RoundMotionSerializer(serializers.ModelSerializer):
        ...
        url = fields.TournamentHyperlinkedRelatedField(
            required=False,
            view_name="api-motion-detail",
    ...
    class RoundLinksSerializer(serializers.Serializer):
        pairing = fields.TournamentHyperlinkedIdentityField(
            view_name="api-pairing-list", lookup_field="seq", lookup_url_kwarg="round_seq"
        )
        availabilities = fields.TournamentHyperlinkedIdentityField(
            view_name="api-availability-list", lookup_field="seq", lookup_url_kwarg="round_seq"
        )
        preformed_panels = fields.TournamentHyperlinkedIdentityField(
            view_name="api-preformedpanel-list", lookup_field="seq", lookup_url_kwarg="round_seq"
        )
    ...
    url = fields.TournamentHyperlinkedIdentityField(
        view_name="api-round-detail", lookup_field="seq", lookup_url_kwarg="round_seq"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not is_staff(kwargs.get("context")):
            with_permission = partial(
                has_permission,
                user=kwargs["context"]["request"].user,
                tournament=kwargs["context"]["tournament"],
            )
            ...
            if not with_permission(permission=Permission.VIEW_MOTION) and (
                isinstance(self.instance, QuerySet) or not self.instance.motions_released
            ):
    ...
    def validate(self, data):
        ...
        if (bc is None) == (stage == Round.Stage.ELIMINATION):
            ...
            raise serializers.ValidationError(
                "Rounds are elimination iff they have a break category."
            )
        return super().validate(data)

    def create(self, validated_data):
        motions_data = validated_data.pop("roundmotion_set", [])
        if len(motions_data) > 0 and not has_permission(
            self.context["request"].user, Permission.EDIT_MOTION, self.context["tournament"]
        ):
    ...
    def update(self, instance, validated_data):
        ...
        for i, roundmotion in enumerate(motions_data, start=1):
            ...
            motion.save()
            RoundMotion.objects.update_or_create(
                round=instance, motion=motion, defaults={"seq": roundmotion["seq"]}
            )
...
class BreakCategorySerializer(serializers.ModelSerializer):
    ...
    url = fields.TournamentHyperlinkedIdentityField(view_name="api-breakcategory-detail")
...
class BreakingTeamSerializer(serializers.ModelSerializer):
    team = fields.TournamentHyperlinkedRelatedField(
        view_name="api-team-detail", queryset=Team.objects.all()
    )
    ...
    def validate_team(self, value):
        qs = BreakingTeam.objects.filter(
            break_category=self.context["break_category"], team=value
        ).exclude(id=getattr(self.instance, "id", None))
        if qs.exists():
...
class SpeakerSerializer(serializers.ModelSerializer):
    ...
    class SpeakerLinksSerializer(serializers.Serializer):
        checkin = fields.TournamentHyperlinkedIdentityField(
            tournament_field="team__tournament", view_name="api-speaker-checkin"
        )

    url = fields.TournamentHyperlinkedIdentityField(
        tournament_field="team__tournament", view_name="api-speaker-detail"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not is_staff(kwargs.get("context")):
            ...
            with_permission = partial(
                has_permission, user=kwargs["context"]["request"].user, tournament=t
            )
            ...
            if (
                not with_permission(permission=Permission.VIEW_PARTICIPANT_DECODED)
                and t.pref("participant_code_names") == "everywhere"
            ):
    ...
    def create(self, validated_data):
        ...
        if (
            url_key is not None and len(url_key) != 0
        ):  # Let an empty string be null for the uniqueness constraint
...
class AdjudicatorSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not is_staff(kwargs.get("context")):
            ...
            if not with_permission(permission=Permission.VIEW_PARTICIPANT_INST) and not t.pref(
                "show_adjudicator_institutions"
            ):
                self.fields.pop("institution")
            if not with_permission(permission=Permission.VIEW_ADJ_BREAK) and not t.pref(
                "public_breaking_adjs"
            ):
                self.fields.pop("breaking")
            if (
                not with_permission(permission=Permission.VIEW_PARTICIPANT_DECODED)
                and not t.pref("participant_code_names") == "everywhere"
            ):
                self.fields.pop("name")
    ...
    def update(self, instance, validated_data):
        ...
        if "base_score" in validated_data and validated_data["base_score"] != instance.base_score:
            AdjudicatorBaseScoreHistory.objects.create(
                ...
                round=self.context["tournament"].current_round,
                score=validated_data["base_score"],
            )

        if self.partial:
            # Avoid removing conflicts if merely PATCHing
            for field in ["institution_conflicts", "adjudicator_conflicts", "team_conflicts"]:
                validated_data[field] = list(getattr(instance, field).all()) + validated_data.get(
                    field, []
                )
...
class TeamSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        self.fields["speakers"] = self.TeamSpeakerSerializer(
            *args, many=True, required=False, **kwargs
        )
        ...
        if not is_staff(kwargs.get("context")):
            ...
            if not with_permission(permission=Permission.VIEW_DECODED_TEAMS) and t.pref(
                "team_code_names"
            ) in ("admin-tooltips-code", "admin-tooltips-real", "everywhere"):
                self.fields.pop("institution")
            ...
            elif not with_permission(permission=Permission.VIEW_PARTICIPANT_INST) and not t.pref(
                "show_team_institutions"
            ):
                self.fields.pop("institution")
                self.fields.pop("use_institution_prefix")
    ...
    def validate(self, data):
        if data.get("institution") is None and data.get("use_institution_prefix", False):
            raise serializers.ValidationError(
                "Cannot include institution prefix without institution."
            )
        ...
        if uniqueness_qs.exists() and not self.partial:
            raise serializers.ValidationError(
                "Team with same reference and institution exists in the tournament"
            )
    ...
    def update(self, instance, validated_data):
        ...
        if self.partial:
            # Avoid removing conflicts if merely PATCHing
            validated_data["institution_conflicts"] = list(
                instance.institution_conflicts.all()
            ) + validated_data.get("institution_conflicts", [])
...
class InstitutionSerializer(serializers.ModelSerializer):
    ...
    region = fields.CreatableSlugRelatedField(
        slug_field="name", queryset=Region.objects.all(), required=False, allow_null=True
    )
    venue_constraints = VenueConstraintSerializer(many=True, required=False)
...
class SpeakerStandingsSerializer(BaseStandingsSerializer):
    speaker = fields.AnonymisingHyperlinkedTournamentRelatedField(
        view_name="api-speaker-detail", anonymous_source="anonymous"
    )
...
class DebateAdjudicatorSerializer(serializers.Serializer):
    ...
    chair = fields.TournamentHyperlinkedRelatedField(
        view_name="api-adjudicator-detail", queryset=adjudicators
    )
    panellists = fields.TournamentHyperlinkedRelatedField(
        many=True, view_name="api-adjudicator-detail", queryset=adjudicators
    )
    trainees = fields.TournamentHyperlinkedRelatedField(
        many=True, view_name="api-adjudicator-detail", queryset=adjudicators
    )
...
class RoundPairingSerializer(serializers.ModelSerializer):
    ...
    class PairingLinksSerializer(serializers.Serializer):
        ballots = fields.RoundHyperlinkedIdentityField(
            view_name="api-ballot-list", lookup_field="pk", lookup_url_kwarg="debate_pk"
        )

    url = fields.RoundHyperlinkedIdentityField(
        view_name="api-pairing-detail", lookup_url_kwarg="debate_pk"
    )
    venue = fields.TournamentHyperlinkedRelatedField(
        view_name="api-venue-detail", queryset=Venue.objects.all(), required=False, allow_null=True
    )
    ...
    def update(self, instance, validated_data):
        for team in validated_data.pop("debateteam_set", []):
            try:
                DebateTeam.objects.update_or_create(
                    debate=instance,
                    side=team.get("side"),
                    defaults={
                        "team": team.get("team"),
...
class FeedbackSerializer(TabroomSubmissionFieldsMixin, serializers.ModelSerializer):
    ...
    class FeedbackAnswerSerializer(serializers.Serializer):
        ...
        def validate(self, data):
            ...
            if type(data["answer"]) != model.ANSWER_TYPE:
                raise serializers.ValidationError(
                    {"answer": "The answer must be of type %s" % model.ANSWER_TYPE.__name__}
                )

            data["answer"] = model.ANSWER_TYPE(data["answer"])

            option_error = serializers.ValidationError(
                {"answer": "Answer must be in set of options"}
            )
            if len(data["question"].choices) > 0:
                if (
                    model.ANSWER_TYPE is list
                    and len(set(data["answer"]) - set(data["question"].choices)) > 0
                ):
                    raise option_error
                if data["answer"] not in data["question"].choices:
                    raise option_error
            if (
                data["question"].min_value is not None
                and data["answer"] < data["question"].min_value
            ) or (
                data["question"].max_value is not None
                and data["answer"] > data["question"].max_value
            ):
                raise option_error
    ...
    adjudicator = fields.TournamentHyperlinkedRelatedField(
        view_name="api-adjudicator-detail", queryset=Adjudicator.objects.all()
    )
    source = SubmitterSourceField(source="*")
    participant_submitter = fields.ParticipantSourceField(allow_null=True)
    debate = DebateHyperlinkedRelatedField(
        view_name="api-pairing-detail", queryset=Debate.objects.all(), lookup_url_kwarg="debate_pk"
    )
    ...
    def validate(self, data):
        ...
        required_questions = self.context["tournament"].adjudicatorfeedbackquestion_set.filter(
            required=True, **{source_type: True}
        )
...
class BallotSerializer(TabroomSubmissionFieldsMixin, serializers.ModelSerializer):
    ...
    class ResultSerializer(serializers.Serializer):
        class SheetSerializer(serializers.Serializer):
            ...
            class TeamResultSerializer(serializers.Serializer):
                ...
                class SpeechSerializer(serializers.Serializer):
                    ghost = serializers.BooleanField(
                        required=False, help_text=SpeakerScore._meta.get_field("ghost").help_text
                    )
                ...
                def validate(self, data):
                    ...
                    if (
                        team is None
                        or len(speakers_team) > 1
                        or (len(speakers_team) == 1 and team.id not in speakers_team)
                    ):
                ...
                def save(self, **kwargs):
                    ...
                    if (
                        result.get_scoresheet_class().uses_declared_winners
                        and self.validated_data.get("win", False)
                    ):
                        args = [self.validated_data["side"]]
                        if (
                            kwargs.get("adjudicator") is not None
                            and not result.ballotsub.single_adj
                        ):
            ...
            def validate_adjudicator(self, value):
                ...
                if (
                    not self.context.get("debate")
                    .debateadjudicator_set.filter(adjudicator=value)
                    .exists()
                ):
            ...
            def save(self, **kwargs):
                ...
                for team in self.validated_data.get("teams", []):
                    ...
                    team_serializer.save(
                        result=kwargs["result"], adjudicator=self.validated_data.get("adjudicator")
                    )
        ...
        def validate(self, data):
            ...
            speaker_order = [
                s["speaker"] for t in data["sheets"][0]["teams"] for s in t.get("speeches", [])
            ]
            for sheet in data["sheets"]:
                speeches = [
                    s["speaker"] for t in sheet.get("teams", []) for s in t.get("speeches", [])
                ]
        ...
        def create(self, validated_data):
            result = DebateResult(
                validated_data["ballot"], tournament=self.context.get("tournament")
            )
    ...
    class VetoSerializer(serializers.ModelSerializer):
        ...
        motion = fields.TournamentHyperlinkedRelatedField(
            view_name="api-motion-detail", queryset=Motion.objects.all()
        )
        ...
        def create(self, validated_data):
            ...
            try:
                validated_data["debate_team"] = DebateTeam.objects.get(
                    debate=self.context["debate"], team=team
                )
    ...
    motion = fields.TournamentHyperlinkedRelatedField(
        view_name="api-motion-detail", required=False, queryset=Motion.objects.all()
    )
    ...
    vetos = VetoSerializer(
        many=True, source="debateteammotionpreference_set", required=False, allow_null=True
    )
    ...
    def create(self, validated_data):
        ...
        if self.context["tournament"].pref("ballots_per_debate_" + stage) == "per-adj":
            debateadj_count = (
                self.context["debate"]
                .debateadjudicator_set.exclude(type=DebateAdjudicator.TYPE_TRAINEE)
                .count()
            )
            if debateadj_count > 1:
                if len(result_data["sheets"]) == 1:
                    validated_data["participant_submitter"] = result_data["sheets"][0][
                        "adjudicator"
                    ]
                    validated_data["single_adj"] = True
                elif validated_data.get("single_adj", False):
                    raise serializers.ValidationError(
                        {"single_adj": "Single-adjudicator ballots can only have one scoresheet"}
                    )
            ...
            raise serializers.ValidationError(
                {"result": "Consensus ballots can only have one scoresheet"}
            )
...
class PreformedPanelSerializer(serializers.ModelSerializer):
    url = fields.RoundHyperlinkedIdentityField(
        view_name="api-preformedpanel-detail", lookup_url_kwarg="debate_pk"
    )
    adjudicators = DebateAdjudicatorSerializer(required=False, allow_null=True)
...
class SpeakerRoundScoresSerializer(serializers.ModelSerializer):
    ...
    speaker = fields.TournamentHyperlinkedIdentityField(
        tournament_field="team__tournament", view_name="api-speaker-detail"
    )
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class TournamentPermissionsSerializer(serializers.Serializer):
        tournament = serializers.HyperlinkedIdentityField(
            view_name="api-tournament-detail",
            lookup_field="slug",
            lookup_url_kwarg="tournament_slug",
        )
        groups = fields.TournamentHyperlinkedRelatedField(
            many=True, view_name="api-group-detail", queryset=Group.objects.all(), default=[]
        )
        permissions = serializers.ListField(
            child=serializers.ChoiceField(choices=Permission.choices), required=False
        )
