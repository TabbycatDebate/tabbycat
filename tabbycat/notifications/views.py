    ...
    def form_valid(self, form: TestEmailForm) -> "HttpResponseRedirect":
        ...
        try:
            ...
            try:
                ...
                smtp_error = str(
                    e.smtp_error
                )  # but just in case it's not, fall back to generic str()
            messages.error(
                self.request,
                _(
                    "The email (SMTP) server returned an error sending the test email: "
            ...
            if e.smtp_code == 550 and "Sender Identity" in smtp_error:
                messages.warning(
                    self.request,
                    _(
                        "Hint: If the error is about sender identity verification in SendGrid, "
                ...
                logger.warning(
                    "Suspected SendGrid sender identity verification error in test email",
                    exc_info=True,
                )
            else:
    ...
    def get_tables(self) -> List[TabbycatTableBuilder]:
        ...
        for notification in notifications:
            ...
            if notification.round is not None:
                ...
                subtitle = _("@ %s") % formats.time_format(
                    timezone.localtime(notification.timestamp), use_l10n=True
                )

            table = TabbycatTableBuilder(
                view=self, title=notification.get_event_display().capitalize(), subtitle=subtitle
            )
            ...
            for sentmessage in notification.sentmessage_set.all():
                emails_recipient.append(
                    escape(sentmessage.recipient.name)
                    if sentmessage.recipient
                    else self.UNKNOWN_RECIPIENT_CELL
                )
                ...
                if len(sentmessage.statuses) > 0:
                    ...
                    emails_time.append(
                        formats.time_format(
                            timezone.localtime(latest_status.timestamp), use_l10n=True
                        )
                    )
                else:
            ...
            table.add_column(
                {"key": "name", "tooltip": _("Participant"), "icon": "user"}, emails_recipient
            )
            table.add_column(
    ...
    def post(self, request: "HttpRequest", *args, **kwargs) -> HttpResponse:
        ...
        for obj in data:
            ...
            if email_id is None:
                continue
            statuses.append(
                EmailStatus(email_id=email_id, timestamp=timestamp, event=obj["event"], data=obj)
            )
...
class BaseSelectPeopleEmailView(
    AdministratorMixin, TournamentMixin, VueTableTemplateView, FormView
):
    ...
    def get_context_data(self, **kwargs) -> Dict[str, Any]:
        ...
        context["sg_webhook"] = EmailStatus.objects.filter(
            email__notification__tournament=self.tournament
        ).exists()
    ...
    def get_queryset(self) -> "QuerySet[Person]":
        ...
        queryset_filter = Q(speaker__team__tournament=self.tournament) | Q(
    ...
    def default_send(
        self, p: Person, default_send_queryset: Optional["QuerySet[Person]"] = None
    ) -> bool:
    ...
    def add_sent_notification(self, email_count: int) -> None:
        text = ngettext(
            "%(email_count)s email has been queued for sending.",
        ...
        if email_count > 0:
            ...
            messages.warning(
                self.request, _("No emails were sent — likely because no recipients were selected.")
            )
    ...
    def get_table(self, mixed_participants: bool = False) -> TabbycatTableBuilder:
        ...
        table.add_column(
            {"key": "name", "tooltip": _("Participant"), "icon": "user"},
            [
                {
                    "text": escape(p.name),
        ...
        table.add_column(
            {"key": "email", "tooltip": _("Email address"), "icon": "mail"},
            [
                {
...
class TournamentTemplateEmailCreateView(TemplateEmailCreateView):
    ...
    def get_default_send_queryset(self) -> "QuerySet[Person]":
        return (
            super()
            .get_default_send_queryset()
            .exclude(
                sentmessage__notification__event=self.event,
                sentmessage__notification__tournament=self.tournament,
            )
        )
...
class RoundTemplateEmailCreateView(TemplateEmailCreateView, RoundMixin):
    ...
    def get_default_send_queryset(self) -> "QuerySet[Person]":
        return (
            ...
            .exclude(
                sentmessage__notification__event=self.event,
                sentmessage__notification__round=self.round,
            )
        )
