...
from .utils import (
    AdjudicatorAssignmentEmailGenerator,
    BallotsEmailGenerator,
    MotionReleaseEmailGenerator,
    NotificationContextGenerator,
    RandomizedUrlEmailGenerator,
    StandingsEmailGenerator,
    TeamDrawEmailGenerator,
    TeamSpeakerEmailGenerator,
)


class NotificationQueueConsumer(SyncConsumer):

    NOTIFICATION_GENERATORS: Dict[
        BulkNotification.EventType, Type[NotificationContextGenerator]
    ] = {
    ...
    def email(
        self, event: Dict[str, Union[str, BulkNotification.EventType, List[int], Dict[str, Any]]]
    ) -> None:
        # Get database objects
        if "debate_id" in event["extra"]:
            debate = Debate.objects.select_related("round__tournament").get(
                pk=event["extra"].pop("debate_id")
            )
            event["extra"]["debate"] = debate
            round = event["extra"]["debate"].round
            t = round.tournament
        elif "round_id" in event["extra"]:
            round = Round.objects.select_related("tournament").get(
                pk=event["extra"].pop("round_id")
            )
        ...
        recipients = Person.objects.filter(
            pk__in=event["send_to"] or [], email__isnull=False
        ).exclude(email="")
        contexts = self.NOTIFICATION_GENERATORS[notification_type].generate(
            to=recipients, **event["extra"]
        )
        ...
        if notification_type is BulkNotification.EventType.BALLOTS_CONFIRMED:
            ...
            bulk_notification = BulkNotification.objects.create(
                event=notification_type, **creation_kwargs
            )
        ...
        for instance, recipient in contexts:
            ...
            email = mail.EmailMultiAlternatives(
                ...
                to=[formataddr((recipient.name, recipient.email))],
                reply_to=reply_to,
                headers={
                    "X-SMTPAPI": json.dumps(
                        {"unique_args": {"hook-id": hook_id}}
                    ),  # SendGrid-specific 'hook-id'
                },
            )
