        ...
        for bc in categories:
            if bc.tournament != tournament:
                self.add_error(
                    ...
                    ValidationError(
                        _(
                            "The team can't be in a break category of a different tournament. Please remove: %(category)s"
                        ),
                        code="invalid_break_category",
                        params={"category": str(bc)},
                    ),
                )
        return categories
...
class TeamAdmin(ModelAdmin):
    form = TeamForm
    list_display = ("long_name", "short_name", "emoji_code", "institution", "tournament")
    search_fields = (
        "reference",
        "short_name",
        "code_name",
    ...
    inlines = (
        SpeakerInline,
        TeamSideAllocationInline,
        VenueConstraintInline,
        AdjudicatorTeamConflictInline,
        TeamInstitutionConflictInline,
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if (
            db_field.name == "break_categories"
            and kwargs.get("initial") is None
            and Tournament.objects.count() == 1
        ):
    ...
    def assign_emoji(self, request, queryset):
        ...
        for tournament, teams in groupby(
            queryset.select_related("tournament").order_by("tournament_id"), lambda t: t.tournament
        ):
...
class AdjudicatorAdmin(ModelAdmin):
    ...
    inlines = (
        AdjudicatorTeamConflictInline,
        AdjudicatorInstitutionConflictInline,
        AdjudicatorAdjudicatorConflictInline,
        AdjudicatorBaseScoreHistoryInline,
        RoundAvailabilityInline,
    )
