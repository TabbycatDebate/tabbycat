...
def team_record_link(context, team, admin, style=True):
    ...
    if admin:
        url = reverse_tournament(
            "participants-team-record", context["tournament"], kwargs={"pk": team.pk}
        )
    else:
        url = reverse_tournament(
            "participants-public-team-record", context["tournament"], kwargs={"pk": team.pk}
        )

    classes = 'class="list-group-item-text alert-link"' if style else ""

    return mark_safe(
        """<a href="%(url)s" %(style)s>%(name)s</a>"""
        % {"url": url, "style": classes, "name": name}
    )
...
def adj_record_link(context, adj, admin):
    ...
    if admin:
        url = reverse_tournament(
            "participants-adjudicator-record", context["tournament"], kwargs={"pk": adj.pk}
        )
    else:
        url = reverse_tournament(
            "participants-public-adjudicator-record", context["tournament"], kwargs={"pk": adj.pk}
        )
