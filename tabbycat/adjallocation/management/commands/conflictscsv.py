...
class Command(TournamentCommand):
    ...
    def handle_tournament(self, tournament, **options):
        ...
        for conflict in AdjudicatorInstitutionConflict.objects.filter(
            adjudicator__tournament=tournament
        ):
            if (
                options["include_own_institution"]
                or conflict.adjudicator.institution != conflict.institution
            ):
        ...
        for conflict in TeamInstitutionConflict.objects.filter(team__tournament=tournament):
            if (
                options["include_own_institution"]
                or conflict.team.institution != conflict.institution
            ):
