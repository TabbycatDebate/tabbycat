...
class Command(RoundCommand):
    ...
    help = (
        "Runs the auto-allocator to assign adjudicators to debates. This "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-p", "--preformed", action="store_true", help="Allocate to preformed panels instead"
        )
        parser.add_argument(
            "-f", "--force", action="store_true", help="Run even if draw status is not confirmed"
        )
        parser.add_argument(
            "-q",
            "--quiet",
    ...
    def handle_round(self, round, **options):
        if (
            not options["preformed"]
            and not options["force"]
            and round.draw_status != Round.Status.CONFIRMED
        ):
            raise CommandError(
                "Draw status isn't confirmed (it's {}), use "
        ...
        if allocator_key == "hungarian":
            allocator_key = (
                ...
                else "hungarian-consensus"
            )
            self.stdout.write(
                "Using allocator: " + allocator_key
            )  # only print if not specified by user
        ...
        if not options["dry_run"]:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "Saved debate adjudicators for {:d} debates.".format(len(allocations))
                )
            )
        else:
            self.stdout.write(
                self.style.MIGRATE_LABEL("Dry run requested, not saving to database.")
            )
