...
class Command(TournamentCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--adjudicators-only", action="store_true", help="Only add for adjudicators, skip teams"
        )
        parser.add_argument(
            "--teams-only", action="store_true", help="Only add for teams, skip adjudicators"
        )
    ...
    def add_for_queryset(self, qs):
        ...
        self.stdout.write(
            "Done, created {missing} previously-missing {model} own-institution conflicts.".format(
                missing=missing, model=qs.model._meta.verbose_name
            )
        )
        self.stdout.write(
            "{existing} {models} already had own-institution conflicts defined.".format(
