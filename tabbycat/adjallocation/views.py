...
from .models import (
    ...
    PreformedPanelAdjudicator,
    TeamInstitutionConflict,
)
from .serializers import (
    EditDebateAdjsDebateSerializer,
    EditPanelAdjsPanelSerializer,
    EditPanelOrDebateAdjSerializer,
)

logger = logging.getLogger(__name__)


class BaseEditDebateOrPanelAdjudicatorsView(
    DebateDragAndDropMixin, AdministratorMixin, TemplateView
):
    ...
    def get_adjudicator_conflicts(self):
        conflicts = ConflictsInfo(
            teams=self.tournament.team_set.all(), adjudicators=self.tournament.adjudicator_set.all()
        )
...
class EditDebateAdjudicatorsView(BaseEditDebateOrPanelAdjudicatorsView):
    ...
    def debates_or_panels_factory(self, debates):
        return EditDebateAdjsDebateSerializer(
            debates, many=True, context={"sides": self.tournament.sides, "round": self.round}
        )
...
class EditPanelAdjudicatorsView(BaseEditDebateOrPanelAdjudicatorsView):
    ...
    def get_extra_info(self):
        info = super().get_extra_info()
        info["backUrl"] = reverse_tournament(
            "panel-adjudicators-index", self.tournament
        )  # Override
    ...
    def debates_or_panels_factory(self, panels):
        return EditPanelAdjsPanelSerializer(panels, many=True, context={"round": self.round})
...
class BaseAdjudicatorConflictsView(
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["can_edit"] = has_permission(
            self.request.user, self.get_edit_permission(), self.tournament
        )
        return super().get_context_data(**kwargs)
...
class AdjudicatorAdjudicatorConflictsView(BaseAdjudicatorConflictsView):
    ...
    def add_message(self, nsaved, ndeleted):
        ...
        if nsaved == 0 and ndeleted == 0:
            messages.success(
                self.request, _("No changes were made to adjudicator-adjudicator conflicts.")
            )
...
class AdjudicatorInstitutionConflictsView(BaseAdjudicatorConflictsView):
    ...
    def add_message(self, nsaved, ndeleted):
        ...
        if nsaved == 0 and ndeleted == 0:
            messages.success(
                self.request, _("No changes were made to adjudicator-institution conflicts.")
            )
