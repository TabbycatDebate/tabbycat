...
class ActionLogEntry(models.Model):
    ...
    class ActionType(models.TextChoices):
        ...
        PREFORMED_PANELS_ADJUDICATOR_AUTO = "pp.aj.auto", _(
            "Auto-allocated adjudicators to preformed panels"
        )
        ...
        PREFORMED_PANELS_DEBATES_AUTO = "pp.db.auto", _(
            "Auto-allocated preformed panels to debates"
        )
    ...
    type = models.CharField(max_length=10, choices=ActionType.choices, verbose_name=_("type"))
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, models.PROTECT, blank=True, null=True, verbose_name=_("user")
    )
    ...
    round = models.ForeignKey(
        "tournaments.Round", models.SET_NULL, blank=True, null=True, verbose_name=_("round")
    )

    # cascade to keep generic foreign keys complete where existent
    content_type = models.ForeignKey(
        ContentType, models.CASCADE, blank=True, null=True, verbose_name=_("content type")
    )
    object_id = models.PositiveIntegerField(blank=True, null=True, verbose_name=_("object ID"))
    content_object = GenericForeignKey("content_type", "object_id")
    agent = models.CharField(
        max_length=1, choices=Agent.choices, default=Agent.WEB, verbose_name=_("agent")
    )
    ...
    def clean(self):
        if self.user is None and self.ip_address is None:
            raise ValidationError(
                _("All log entries require at least one of a user and an IP address.")
            )
