...
class Permission(TextChoices):
    ...
    VIEW_ADJ_ADJ_CONFLICTS = "view.adjudicatoradjudicatorconflict", _(
        "view adjudicator-adjudicator conflicts"
    )
    EDIT_ADJ_ADJ_CONFLICTS = "edit.adjudicatoradjudicatorconflict", _(
        "edit adjudicator-adjudicator conflicts"
    )
    VIEW_ADJ_INST_CONFLICTS = "view.adjudicatorinstitutionconflict", _(
        "view adjudicator-institution conflicts"
    )
    EDIT_ADJ_INST_CONFLICTS = "edit.adjudicatorinstitutionconflict", _(
    ...
    VIEW_PARTICIPANT_CONTACT = "view.participants.contact", _(
    ...
    VIEW_ROUNDAVAILABILITIES_TEAM = "view.roundavailability.team", _(
        "view round availabilities for teams"
    )
    VIEW_ROUNDAVAILABILITIES_ADJ = "view.roundavailability.adjudicator", _(
        "view round availabilities for adjudicators"
    )
    VIEW_ROUNDAVAILABILITIES_VENUE = "view.roundavailability.venue", _(
        "view round availabilities for rooms"
    )
    EDIT_ROUNDAVAILABILITIES_TEAM = "edit.roundavailability.team", _(
        "edit round availabilities for teams"
    )
    EDIT_ROUNDAVAILABILITIES_ADJ = "edit.roundavailability.adjudicator", _(
        "edit round availabilities for adjudicators"
    )
    EDIT_ROUNDAVAILABILITIES_VENUE = "edit.roundavailability.venue", _(
    ...
    MARK_OTHERS_BALLOTSUBMISSIONS = "mark.ballotsubmission.others", _(
    ...
    VIEW_TOURNAMENTPREFERENCEMODEL = "view.tournamentpreferencemodel", _(
        "view tournament configuration"
    )
    EDIT_TOURNAMENTPREFERENCEMODEL = "edit.tournamentpreferencemodel", _(
...
def has_permission(
    user: "settings.AUTH_USER_MODEL", permission: permission_type, tournament: "Tournament"
) -> bool:
    ...
    perm = (
        user.userpermission_set.filter(permission=permission, tournament=tournament).exists()
        or user.membership_set.filter(
            group__permissions__contains=[permission], group__tournament=tournament
        ).exists()
    )
    if perm:
        user._permissions[tournament.slug].add(permission)
...
def get_permissions(user: "settings.AUTH_USER_MODEL") -> List["Tournament"]:
    user_perms = {}
    for t, groups in groupby(
        user.membership_set.select_related("group", "group__tournament")
        .order_by("group__tournament")
        .all(),
        key=lambda m: m.group.tournament,
    ):
    ...
    for t, perms in groupby(
        user.userpermission_set.select_related("tournament").order_by("tournament").all(),
        key=lambda p: p.tournament,
    ):
        tournament = user_perms.setdefault(t.id, t)
        tournament.permissions = getattr(tournament, "permissions", set()) | {
            p.permission for p in perms
        }
