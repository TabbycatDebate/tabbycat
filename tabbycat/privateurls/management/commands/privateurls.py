...
class Command(TournamentCommand):
    ...
    def add_arguments(self, parser):
        subparsers = parser.add_subparsers(
            dest="subcommand", parser_class=ArgumentParser, metavar="{generate,delete}"
        )
        ...
        generate.add_argument(
            "-l", "--length", type=int, default=8, help="Length of URL key to generate (default 8)"
        )
        generate.add_argument(
            "-O",
            "--overwrite",
    ...
    def populate_url_keys(self, relatedmanager):
        ...
        if existing.exists():
            self.stdout.write(
                self.style.WARNING(
                    "Skipping {0:d} {1:s} that already have private URLs. Use --overwrite to overwrite them.".format(
        ...
        self.stdout.write(
            "Generating private URLs for {0:d} {1:s}".format(queryset.count(), model_name)
        )
