...
class PrivateUrlSentMailRecord(models.Model):
    ...
    def __str__(self):
        return "{x.recipient.name:s} ({x.email:s}): {x.url_key:s}".format(x=self)

    @property
    def recipient(self):
        if self.adjudicator and self.speaker:
            raise ValueError("A sent mail record had both a recipient adjudicator and a recipient speaker.")
        return self.adjudicator or self.speaker
