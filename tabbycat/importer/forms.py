    ...
    def clean_institutions_raw(self):
        ...
        if len(institutions) > max_allowed:
            raise ValidationError(
                ...
                % {"max_allowed": max_allowed, "given": len(institutions)}
            )
    ...
    def clean_venues_raw(self):
        ...
        if len(venues) > max_allowed:
            raise ValidationError(
                ...
                % {"max_allowed": max_allowed, "given": len(venues)}
            )
...
class BaseInstitutionObjectDetailsForm(BaseTournamentObjectDetailsForm):
    ...
    institution = forms.ModelChoiceField(
        queryset=Institution.objects.all(), widget=forms.HiddenInput, required=False
    )
    ...
    def __init__(self, tournament, *args, **kwargs):
        ...
        institution_id = self.initial.get("institution") or self.data.get(
            self.add_prefix("institution")
        )
...
class TeamDetailsForm(BaseInstitutionObjectDetailsForm):
    ...
    speakers = forms.CharField(
        required=True,
        label=_("Speakers' names"),
        help_text=_("Can be separated by newlines, tabs or commas"),
    )
    emails = forms.CharField(
        required=False,
        label=_("Speakers' email addresses"),
        help_text=_(
            "Optional, useful to include if distributing private URLs, list in same order as speakers' names"
        ),
    )
    short_reference = forms.CharField(
        widget=forms.HiddenInput, required=False
    )  # doesn't actually do anything, just placeholder to avoid validation failure
    ...
    class Meta:
        ...
        help_texts = {
            "reference": _(
                'Do not include institution name (check the "Prefix team name with institution name?" field instead)'
            ),
        }
    ...
    def __init__(self, tournament, *args, **kwargs):
        ...
        if self.institution_for_display is None:
            ...
            self.fields["use_institution_prefix"].help_text = _(
                "(Not applicable to unaffiliated teams)"
            )

        # Set speaker and email widgets to match tournament settings
        nspeakers = tournament.pref("substantive_speakers")
        self.fields["speakers"].widget = forms.Textarea(
            attrs={"rows": nspeakers, "placeholder": _("One speaker's name per line")}
        )
        self.initial.setdefault(
            "speakers", "\n".join(_("Speaker %d") % i for i in range(1, nspeakers + 1))
        )
        self.fields["emails"].widget = forms.Textarea(
            attrs={
                "rows": nspeakers,
                "placeholder": "\n".join(
                    _("speaker%d@example.edu") % i for i in range(1, nspeakers + 1)
                ),
            }
        )
        ...
        if tournament.pref("show_seed_in_importer") == "numeric":
            self.fields["seed"] = forms.IntegerField(
                required=False, label=seed_label, help_text=seed_help, min_value=0
            )
        elif tournament.pref("show_seed_in_importer") == "title":
            self.fields["seed"] = forms.ChoiceField(
                required=False,
                label=seed_label,
                choices=(
                    (0, _("Unseeded")),
    ...
    def clean_emails(self):
        ...
        for email in emails:
            try:
                ...
                self.add_error(
                    "emails", _("%(email)s is not a valid email address.") % {"email": email}
                )
        return emails
    ...
    def save(self, commit=True):
        ...
        if commit:
            team.save()
            for name, email in zip_longest(
                self.cleaned_data["speakers"], self.cleaned_data["emails"]
            ):
...
class TeamDetailsFormSet(forms.BaseModelFormSet):
    ...
    def get_unique_error_message(self, unique_check):
        ...
        if unique_check == ("reference", "institution", "tournament"):
            return _(
                "Every team in a single tournament from the same institution must "
...
class AdjudicatorDetailsForm(BaseInstitutionObjectDetailsForm):
    ...
    def clean_base_score(self):
        ...
        if base_score < min_score or max_score < base_score:
            self.add_error(
                "base_score",
                _("This value must be between %(min)d and %(max)d.")
...
class BaseNumberForEachInstitutionForm(forms.Form):
    ...
    number_unaffiliated = forms.IntegerField(
        min_value=0,
        required=False,
    ...
    def clean(self):
        ...
        for institution in self.institutions:
            given += (
                self.cleaned_data.get("number_institution_%d" % institution.id) or 0
            )  # data might be None
        max_allowed = MAX_FORM_DATA_FIELDS // self.num_detail_fields
        if given > max_allowed:
            raise ValidationError(
                self.too_many_error_message % {"max_allowed": max_allowed, "given": given}
            )
