...
from options.presets import (
    AustralianEastersPreferences,
    AustralsPreferences,
    BritishParliamentaryPreferences,
    CanadianParliamentaryPreferences,
    JoyntPreferences,
    NZEastersPreferences,
    UADCPreferences,
    WSDCPreferences,
)
...
class Exporter:
    ...
    def add_rounds(self):
        results_prefetch = Prefetch(
            ...
            queryset=BallotSubmission.objects.filter(confirmed=True).prefetch_related(
                "speakerscore_set", "speakerscorebyadj_set", "teamscore_set"
            ),
        )
        veto_prefetch = Prefetch(
            "debateteammotionpreference_set",
            queryset=DebateTeamMotionPreference.objects.filter(
                preference=3,
                ballot_submission__confirmed=True,
            ),
        )
        dt_prefetch = Prefetch(
            "debateteam_set",
        ...
        debate_prefetch = Prefetch(
            "debate_set",
            queryset=Debate.objects.all().prefetch_related(
        ...
        for round in (
            ...
            if round.stage == Round.Stage.ELIMINATION:
                round_tag.set(
                    "break-category", BREAK_CATEGORY_PREFIX + str(round.break_category_id)
                )
    ...
    def add_debates(self, round_tag, motion, debate):
        ...
        adjs = " ".join(
            [ADJ_PREFIX + str(d_adj.adjudicator_id) for d_adj in debate.debateadjudicator_set.all()]
        )
        if adjs != "":
            debate_tag.set("adjudicators", adjs)

            chair = debate.debateadjudicator_set.get(
        ...
        if debate.confirmed_ballot is not None:
            ...
            for side in self.t.sides:
                ...
                if dt.debateteammotionpreference_set.exists():
                    side_tag.set(
                        "motion-veto",
                        MOTION_PREFIX + str(dt.debateteammotionpreference_set.first().motion_id),
                    )
    ...
    def add_team_ballots(self, side_tag, result, adj, scoresheet, side):
        ...
        if result.is_voting:
            ...
            ballot_tag.set(
                "ignored", str(minority and not self.t.pref("margin_includes_dissenters")).lower()
            )
        else:
                ...
                if result.is_voting:
                    ...
                    ballot_tag = SubElement(
                        ...
                        "ballot",
                        {
                            "adjudicators": " ".join(
                                [
                                    ADJ_PREFIX + str(d_adj.adjudicator_id)
                                    for d_adj in debate.debateadjudicator_set.all()
                                ]
                            ),
                        },
                    )
    ...
    def add_participants(self):
        ...
        speaker_category_prefetch = Prefetch(
            "speaker_set", queryset=Speaker.objects.all().prefetch_related("categories")
        )
        for team in self.t.team_set.all().prefetch_related(
            speaker_category_prefetch, "break_categories"
        ):
            team_tag = SubElement(
                participants_tag,
                "team",
            ...
            team_tag.set(
                "break-eligibilities",
                " ".join(
                    [BREAK_CATEGORY_PREFIX + str(bc.id) for bc in team.break_categories.all()]
                ),
            )

            for speaker in team.speaker_set.all():
                speaker_tag = SubElement(
                    team_tag,
                    "speaker",
                    {
                ...
                speaker_tag.set(
                    "categories",
                    " ".join(
                        [SPEAKER_CATEGORY_PREFIX + str(sc.id) for sc in speaker.categories.all()]
                    ),
                )
        ...
        for adj in self.t.relevant_adjudicators.prefetch_related("adjudicatorfeedback_set"):
            ...
            for feedback in adj.adjudicatorfeedback_set.filter(confirmed=True):
                ...
                if feedback.source_adjudicator is not None:
                    feedback_tag.set(
                        "source-adjudicator",
                        ADJ_PREFIX + str(feedback.source_adjudicator.adjudicator_id),
                    )
                    feedback_tag.set(
                        "debate", DEBATE_PREFIX + str(feedback.source_adjudicator.debate_id)
                    )
                else:
...
class Importer:
    ...
    def import_tournament(self):
        ...
        self.is_bp = (
            self.root.get("style") == "bp" or len(self.root.findall("round[1]/debate[1]/side")) == 4
        )
    ...
    def set_preferences(self):
        ...
        if self.root.get("style") is not None and styles[self.root.get("style", "")] is not None:
            ...
            self.preliminary_consensus = (
                style.debate_rules__ballots_per_debate_prelim == "per-debate"
            )
        ...
        if self.is_bp:
            ...
            substantive_speakers = len(
                self.root.findall("round[1]/debate[1]/side[1]/speech[@reply='false']")
            )
            reply_scores_enabled = (
                len(self.root.findall("round/debate/side/speech[@reply='true']")) != 0
            )
            margin_includes_dissenters = (
                len(
                    self.root.findall("round/debate/side/ballot[@minority='true'][@ignored='true']")
                )
                == 0
            )
            ...
            self.tournament.preferences["debate_rules__ballots_per_debate_prelim"] = (
                "per-debate" if self.preliminary_consensus else "per-adj"
            )
            self.tournament.preferences["debate_rules__ballots_per_debate_elim"] = (
                "per-debate" if self.elimination_consensus else "per-adj"
            )
            self.tournament.preferences["scoring__margin_includes_dissenters"] = (
                margin_includes_dissenters
            )
    ...
    def import_venues(self):
        ...
        for venue in self.root.findall("venue"):
            v = Venue(
                tournament=self.tournament, name=venue.text, priority=venue.get("priority", 0)
            )
            v.save()
    ...
    def import_adjudicators(self):
        ...
        for adj in self.root.findall("participants/adjudicator"):
            adj_obj = Adjudicator(
                ...
                independent=adj.get("independent", False) == "true",
            ...
            adj_obj.institution_conflicts.set(
                [self.institutions[i] for i in adj.get("institutions", "").split(" ") if i != ""]
            )
            adj_obj.team_conflicts.set(
                [self.teams[t] for t in adj.get("team-conflicts", "").split(" ") if t != ""]
            )
    ...
    def import_debates(self):
        ...
        for i, round in enumerate(self.root.findall("round"), 1):
            round_stage = (
                ...
                if round.get("elimination", "false") == "true"
                else Round.Stage.PRELIMINARY
            )
            draw_type = (
                Round.DrawType.ELIMINATION
                if round_stage == Round.Stage.ELIMINATION
                else Round.DrawType.MANUAL
            )
            ...
            round_obj = Round(
                ...
                completed=True,
                name=round.get("name"),
                abbreviation=round.get("abbreviation", round.get("name")[:10]),
                stage=round_stage,
                draw_type=draw_type,
            ...
            for debate in round.findall("debate"):
                ...
                for j, side in enumerate(debate.findall("side"), side_start):
                    ...
                    debateteam_obj = DebateTeam(
                        debate=debate_obj, team=self.teams[side.get("team")], side=position
                    )
                    debateteam_obj.save()
                ...
                for adj in debate.get("adjudicators").split():
                    adj_type = (
                        ...
                        if adj in voting_adjs
                        else DebateAdjudicator.TYPE_TRAINEE
                    )
                    if debate.get("chair") == adj:
                        adj_type = DebateAdjudicator.TYPE_CHAIR
                    adj_obj = DebateAdjudicator(
                        debate=debate_obj, adjudicator=self.adjudicators[adj], type=adj_type
                    )
                    adj_obj.save()
    ...
    def import_motions(self):
        ...
        for r_obj, round in zip(
            self.tournament.round_set.all().order_by("seq"), self.root.findall("round")
        ):
        ...
        for motion in self.root.findall("motion"):
            motion_obj = Motion(
                ...
                info_slide=getattr(motion.find("info-slide"), "text", ""),
                tournament=self.tournament,
            )
            motion_obj.save()
    ...
    def import_results(self):
        for round in self.root.findall("round"):
            consensus = (
                self.preliminary_consensus
                if round.get("elimination") == "false"
                else self.elimination_consensus
            )
                        ...
                        if numeric_scores:
                            ...
                            if consensus:
                                ...
                                for ballot in speech.findall("ballot"):
                                    for adj in [
                                        self.adjudicators[a]
                                        for a in ballot.get("adjudicators", "").split(" ")
                                    ]:
        ...
        for adj in self.root.findall("participants/adjudicator"):
            ...
            for feedback in adj.findall("feedback"):
                d_adj = self.debateadjudicators.get(
                    (feedback.get("debate"), feedback.get("source-adjudicator"))
                )
