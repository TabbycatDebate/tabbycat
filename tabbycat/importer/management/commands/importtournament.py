...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "items", help="Items to import (default: import all)", nargs="*", default=[]
        )
    ...
    def handle(self, *args, **options):
        ...
        self.importer = importer_class(
            ...
            expect_unique=not options["keep_existing"],
        )
    ...
    def get_importer_class(self):
        ...
        if not os.path.exists(importer_spec_filepath) and importer_spec_arg is None:
            raise CommandError(
                "The --importer option wasn't specified and the file "
        ...
        if importer_spec_arg is not None:
            if importer_spec is not None and importer_spec_arg != importer_spec:
                self._warning(
                    "Using importer %s, but data directory suggests "
        ...
        if self.verbosity > 0:
            ...
            if errors:
                for message in errors.itermessages():
                    ...
                    self.stdout.write(message)
            count_strs = (
                "{1:d} {0}".format(model._meta.verbose_name_plural, count)
    ...
    def resolve_tournament_fields(self):
        ...
        short_name = self.options["short_name"] or (
            basename[:24] + ".." if len(basename) > 24 else basename
        )
    ...
    def check_existing_tournament(self, slug):
        ...
        if exists and not self.options["keep_existing"] and not self.options["items"]:
            ...
            DebateTeam.objects.filter(
                team__tournament__slug=slug
            ).delete()  # protected from cascade deletion
