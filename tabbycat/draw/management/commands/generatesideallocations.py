...
class Command(RoundCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--delete", action="store_true", help="Delete allocations, don't create any."
        )
        parser.add_argument(
            "-q", "--quiet", action="store_true", help="Don't print the allocations."
        )
    ...
    def handle_round(self, round, **options):
        ...
        if not options["quiet"]:
            ...
            for aff, neg in zip(
                sorted(affs, key=attrgetter("short_name")),
                sorted(negs, key=attrgetter("short_name")),
            ):
