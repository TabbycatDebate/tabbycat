...
from adjallocation.serializers import (
    SimpleDebateAllocationSerializer,
    SimpleDebateImportanceSerializer,
)
...
class BaseAdjudicatorContainerConsumer(
    SuperuserRequiredWebsocketMixin, RoundWebsocketMixin, JsonWebsocketConsumer
):
...
class DebateEditConsumer(BaseAdjudicatorContainerConsumer):
    ...
    def receive_json(self, content):
        for key in content.keys():
            if key == "sides_confirmed":
                self.receive_debate_change(
                    content, key, "sides_confirmed", "sides_confirmed", self.sides_status_serializer
                )
            elif key == "venues":
                self.receive_debate_change(
                    content, key, "venue", "venue_id", self.venues_serializer
                )
            elif key == "teams":
    ...
    def modify_debate_teams(self, debate, sent_teams):
        ...
        delete_count, deleted = debate.debateteam_set.exclude(
            side__in=self.tournament.sides
        ).delete()
        logger.debug(
            "Deleted %d debate teams from [%s]", deleted.get("draw.DebateTeam", 0), debate.matchup
        )
    ...
    def receive_teams(self, content):
        ...
        debates = self.get_debates_or_panels(changes)
        serialized = self.teams_serializer(
...
class EditDebateOrPanelWorkerMixin(SyncConsumer):
    ...
    def return_response(self, serialized_debates_or_panels, group_name, message_text, message_type):
        content = {
