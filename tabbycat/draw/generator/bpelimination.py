...
class BaseBPEliminationDrawGenerator(EliminationDrawMixin, BaseBPDrawGenerator):
    ...
    def _four_way_fold(self, teams, start_rank=0):
        ...
        if len(teams) % 4 != 0:
            raise DrawFatalError(
                "Tried to do a four-way fold with non-multiple of four: %d" % len(teams)
            )
    ...
    def _get_advancing_teams(self):
        ...
        if advancing_counts.count(0) > 0:
            raise DrawUserError(
                _("%d debates in the previous round don't have a result.")
                % advancing_counts.count(0)
            )
        if advancing_counts.count(2) != len(advancing_counts):
            raise DrawUserError(
                _("%d debates in the previous round don't have exactly two " "teams advancing.")
                % (len(advancing_counts) - advancing_counts.count(2))
            )
        return advancing
...
class PartialBPEliminationDrawGenerator(BaseBPEliminationDrawGenerator):
    ...
    def make_pairings(self):
        ...
        if nteams % 6 != 0 or not ispow2(nteams // 6):
            ...
            raise DrawFatalError(
                "Tried to do a partial elimination draw with invalid break size: %d" % nteams
            )
...
class AfterPartialBPEliminationDrawGenerator(BaseBPEliminationDrawGenerator):
    ...
    def make_pairings(self):
        ...
        if nbypassing % 2 != 0:
            raise DrawUserError(
                _(
                    "The room ranks of the partial elimination round indicate that "
                    "an odd number of teams (%(nbypassing)d) bypassed it."
                )
                % {"nbypassing": nbypassing}
            )
        ndebates = nbypassing // 2
        if len(self.results) != ndebates:
            raise DrawUserError(
                _(
                    "The room ranks of the partial elimination round indicate "
        ...
        if len(advancing) != ndebates:
            ...
            raise DrawFatalError(
                "%d advancing pairs, but %d debates from bypassing pairs"
                % (len(advancing), ndebates)
            )

        pairings = list()
        for i, (team1, team2, adv) in enumerate(
            zip(bypassing_top, bypassing_bottom, advancing), start=1
        ):
...
class FirstBPEliminationDrawGenerator(BaseBPEliminationDrawGenerator):
    ...
    def make_pairings(self):
        ...
        if nteams % 4 != 0 or not ispow2(nteams // 4):
            # This should have been caught by the draw manager
            raise DrawFatalError(
                "Tried to do a first elimination draw with invalid break size: %d" % nteams
            )
...
class SubsequentBPEliminationDrawGenerator(BaseBPEliminationDrawGenerator):
    ...
    def make_pairings(self):
        ...
        if not (len(advancing) >= 2 and ispow2(len(advancing))):
            raise DrawUserError(
                _("The number of debates (%d) in the last round is not a " "power of two.")
                % (2 * len(advancing))
            )
