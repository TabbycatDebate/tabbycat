...
class Submission(models.Model):
    ...
    submitter_type = models.CharField(
        max_length=1, choices=Submitter.choices, verbose_name=_("submitter type")
    )
    confirmed = models.BooleanField(default=False, verbose_name=_("confirmed"))

    # relevant for private URL submissions
    private_url = models.BooleanField(default=False, verbose_name=_("from private URL"))
    participant_submitter = models.ForeignKey(
        "participants.Person",
        models.PROTECT,
        blank=True,
        null=True,
        related_name="%(app_label)s_%(class)s_participant_submitted",
    ...
    confirm_timestamp = models.DateTimeField(
        blank=True, null=True, verbose_name=_("confirm timestamp")
    )
    ...
    def _unique_filter_args(self):
        return dict(
            ...
            for arg in self._meta.constraints[0].fields
            if arg != "version"
        )
    ...
    def save(self, *args, **kwargs):
        ...
        with self.save_lock:
            ...
            if self.confirmed:
                unconfirmed = (
                    self.__class__.objects.filter(confirmed=True, **self._unique_unconfirm_args())
                    .exclude(pk=self.pk)
                    .update(confirmed=False)
                )
                if unconfirmed > 0:
                    logger.info(
                        "Unconfirmed %d %s so that %s could be confirmed",
                        unconfirmed,
...
class BallotSubmission(Submission):
    ...
    debate = models.ForeignKey(
        "draw.Debate", models.CASCADE, db_index=True, verbose_name=_("debate")
    )
    motion = models.ForeignKey(
        "motions.Motion", models.SET_NULL, blank=True, null=True, verbose_name=_("motion")
    )
    discarded = models.BooleanField(default=False, verbose_name=_("discarded"))
    single_adj = models.BooleanField(
        default=False,
        verbose_name=_("single adjudicator"),
        help_text=_(
            "Whether this submission represents only the submitting adjudicator on a panel, "
    ...
    def __str__(self):
        if self.timestamp is None:
            return "[{0.id}] Ballot for {0.debate!s}, no submission time (v{0.version})".format(
                self
            )
        else:
            return (
                "[{0.id}] Ballot for {0.debate!s}, submitted at "
                "{0.timestamp:%Y-%m-%dT%H:%M:%S} (v{0.version})"
            ).format(self)
    ...
    def clean(self):
        ...
        if self.motion is not None and self.debate.round not in self.motion.rounds.all():
            raise ValidationError(
                _("Debate is in round %(round)d but motion (%(motion)s) is not in round")
                % {"round": self.debate.round, "motion": self.motion.reference}
            )
    ...
    def roundmotion(self):
        if not hasattr(self, "_roundmotion"):
            if self.motion is not None:
                self._roundmotion = RoundMotion.objects.get(
                    motion=self.motion, round_id=self.debate.round_id
                )
            else:
...
class TeamScoreByAdj(models.Model):
    ...
    ballot_submission = models.ForeignKey(
        BallotSubmission, models.CASCADE, verbose_name=_("ballot submission")
    )
    debate_adjudicator = models.ForeignKey(
        "adjallocation.DebateAdjudicator", models.CASCADE, verbose_name=_("debate adjudicator")
    )
    debate_team = models.ForeignKey(
        "draw.DebateTeam", models.CASCADE, verbose_name=_("debate team")
    )

    win = models.BooleanField(null=True, blank=True, verbose_name=_("win"))
    margin = ScoreField(null=True, blank=True, verbose_name=_("margin"))
    ...
    def __str__(self):
        ...
        return (
            "[{0.ballot_submission_id}/{0.id}] {1} for "
            "{0.debate_team!s} from {0.debate_adjudicator!s}"
        ).format(self, has_won)
    ...
    def clean(self):
        super().clean()
        if (
            self.debate_team.debate != self.debate_adjudicator.debate
            or self.debate_team.debate != self.ballot_submission.debate
        ):
            raise ValidationError(
                _(
                    "The debate team, debate adjudicator and ballot "
...
class SpeakerScoreByAdj(models.Model):
    ...
    class Meta:
        constraints = [
            UniqueConstraint(
                fields=["debate_adjudicator", "debate_team", "position", "ballot_submission"]
            ),
        ]
        indexes = [models.Index(fields=["ballot_submission", "debate_adjudicator"])]
    ...
    def __str__(self):
        return (
            "[{0.ballot_submission_id}/{0.id}] {0.score} at {0.position} for "
            "{0.debate_team!s} from {0.debate_adjudicator!s}"
        ).format(self)
...
class TeamScore(models.Model):
    ...
    debate_team = models.ForeignKey(
        "draw.DebateTeam", models.CASCADE, db_index=True, verbose_name=_("debate team")
    )

    points = models.PositiveSmallIntegerField(null=True, blank=True, verbose_name=_("points"))
    win = models.BooleanField(null=True, blank=True, verbose_name=_("win"))
    margin = ScoreField(null=True, blank=True, verbose_name=_("margin"))
    score = ScoreField(null=True, blank=True, verbose_name=_("score"))
    votes_given = models.PositiveSmallIntegerField(
        null=True, blank=True, verbose_name=_("votes given")
    )
    votes_possible = models.PositiveSmallIntegerField(
        null=True, blank=True, verbose_name=_("votes possible")
    )
    ...
    def __str__(self):
        return (
            "[{0.ballot_submission_id}/{0.id}] {0.points}, {0.score} for " "{0.debate_team!s}"
        ).format(self)
...
class SpeakerScore(models.Model):
    ...
    speaker = models.ForeignKey(
        "participants.Speaker", models.CASCADE, db_index=True, verbose_name=_("speaker")
    )
    rank = models.PositiveSmallIntegerField(null=True, blank=True, verbose_name=_("rank"))
    score = ScoreField(verbose_name=_("score"))
    position = models.IntegerField(verbose_name=_("position"))
    ...
    def __str__(self):
        return (
            "[{0.ballot_submission_id}/{0.id}] {0.score} at {0.position} for "
            "{0.speaker.name} in {0.debate_team!s}"
        ).format(self)

    def clean(self):
        super().clean()
        if self.debate_team.team != self.speaker.team:
            raise ValidationError(_("The debate team and speaker must be from the " "same team."))
        if self.ballot_submission.debate != self.debate_team.debate:
            raise ValidationError(
                _("The ballot submission and debate team must " "relate to the same debate.")
            )
