...
scores = [
    [
        [float(random.randint(70, 80)) for pos in range(SPEAKERS_PER_TEAM)]
        + [float(random.randint(70, 80)) / 2]
...
common_fields["everyone_scores"] = [
    [mean(adj[team][pos] for adj in scores) for pos in range(SPEAKERS_PER_TEAM + 1)]
    for team in range(TEAMS_PER_DEBATE)
]
...
testdata["high-required"] = {
    "winner_by_adj": [
        "aff" if (adj[0] > adj[1]) else "neg" if (adj[1] > adj[0]) else None
...
testdata["tied-allowed"] = {
    "winner_by_adj": [
        (
            "aff"
            if (adj[0] >= adj[1]) and declared == "aff"
            else "neg" if (adj[1] >= adj[0]) and declared == "neg" else None
        )
        for adj, declared in zip(totals_by_adj, declared_winners)
    ],
}
...
for scoresheet_type in ["high-required", "low-allowed", "tied-allowed"]:
    ...
    fields["winner"] = winner = (
        "aff" if (winner_by_adj.count("aff") > winner_by_adj.count("neg")) else "neg"
    )

    # Scores excluding dissenters
    majority = [adj for winner_by_adj, adj in zip(winner_by_adj, scores) if winner_by_adj == winner]
    fields["majority_adjs"] = [
        i for i, winner_by_adj in enumerate(winner_by_adj) if winner_by_adj == winner
    ]
    fields["majority_scores"] = [
        [mean(adj[team][pos] for adj in majority) for pos in range(SPEAKERS_PER_TEAM + 1)]
        for team in range(TEAMS_PER_DEBATE)
    ]
