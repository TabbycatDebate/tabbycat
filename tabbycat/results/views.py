...
from tournaments.mixins import (
    CurrentRoundMixin,
    PersonalizablePublicTournamentPageMixin,
    PublicTournamentPageMixin,
    RoundMixin,
    SingleObjectByRandomisedUrlMixin,
...
from .forms import (
    ...
    PerAdjudicatorEliminationBallotSetForm,
    SingleBallotSetForm,
...
class PublicResultsIndexView(PublicTournamentPageMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        kwargs["rounds"] = self.tournament.round_set.filter(completed=True, silent=False).order_by(
            "seq"
        )
...
class BaseResultsEntryForRoundView(RoundMixin, VueTableTemplateView):
    ...
    def get_context_data(self, **kwargs):
        kwargs["incomplete_ballots"] = (
            self._get_draw()
            .filter(Q(result_status=Debate.STATUS_NONE) | Q(result_status=Debate.STATUS_DRAFT))
            .exists()
        )
...
class AdminResultsEntryForRoundView(AdministratorMixin, BaseResultsEntryForRoundView):
    ...
    def get_context_data(self, **kwargs):
        ...
        multiple_confirmed_ballots_found = (
            self.round.debate_set.annotate(
                num_ballots=Count("ballotsubmission", filter=Q(ballotsubmission__confirmed=True)),
            )
            .filter(num_ballots__gt=1)
            .exists()
        )
...
class PublicResultsForRoundView(RoundMixin, PublicTournamentPageMixin, VueTableTemplateView):
    ...
    def get_table_by_debate(self):
        debates = self.round.debate_set_with_prefetches(
            results=True, wins=True, institutions=True, adjudicators=True
        )
        populate_confirmed_ballots(
            debates, motions=True, results=self.round.ballots_per_debate == "per-adj"
        )
    ...
    def get_table_by_team(self):
        teamscores = (
            TeamScore.objects.filter(
                debate_team__debate__round=self.round, ballot_submission__confirmed=True
            )
            .prefetch_related(
            ...
            .select_related(
                "ballot_submission", "debate_team__team__institution", "debate_team__debate__round"
            )
        )
...
class BaseBallotSetView(LogActionMixin, TournamentMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["iron"] = self.debate.debateteam_set.annotate(
            iron=Count(
                "team__debateteam__teamscore",
                filter=Q(
                    team__debateteam__debate__round=self.debate.round.prev,
        ...
        if self.ballotsub.id is None:
            ...
            kwargs["current_iron"] = (
                self.debate.debateteam_set.annotate(
                    iron=Count(
                        "team__debateteam__teamscore",
                        filter=Q(
    ...
    def get_all_ballotsubs(self):
        all_ballotsubs = self.debate.ballotsubmission_set.order_by("version").select_related(
            "submitter", "confirmer", "motion"
        )
    ...
    def should_send_email_receipts(self):
        return self.tournament.pref("enable_ballot_receipts") and not (
            self.debate.round.stage == Round.Stage.ELIMINATION
            and self.tournament.pref("teams_in_debate") == "bp"
        )
...
class BaseNewBallotSetView(SingleObjectFromTournamentMixin, BaseBallotSetView):
    ...
    def populate_objects(self, prefill=True):
        ...
        if (
            self.debate.round.ballots_per_debate == "per-adj"
            and not self.debate.adjudicators.has_chair
        ):
            messages.error(
                self.request,
                _(
                    "Whoops! The debate %(debate)s doesn't have a chair, "
        ...
        if (
            not (
                self.tournament.pref("draw_side_allocations") == "manual-ballot"
                and self.tournament.pref("teams_in_debate") == "two"
            )
            and not self.debate.sides_confirmed
        ):
            messages.error(
                self.request,
                _(
                    "Whoops! The debate %(debate)s doesn't have its "
...
class BasePublicNewBallotSetView(
    PersonalizablePublicTournamentPageMixin, RoundMixin, BaseBallotSetView
):
    ...
    def add_success_message(self):
        messages.success(
            self.request,
            _("Thanks, %(user)s! Your ballot for %(debate)s has " "been recorded.")
            % {"user": self.object.name, "debate": self.matchup_description()},
        )
    ...
    def populate_objects(self, prefill=True):
        ...
        if (
            self.tournament.pref("enable_motions") or self.tournament.pref("motion_vetoes_enabled")
        ...
        try:
            ...
            return self.error_page(
                _(
                    "It looks like you're assigned to two or more debates this round. "
        ...
        if self.ballotsub.single_adj and prefill:
            former_ballot = (
                self.debate.ballotsubmission_set.filter(discarded=False)
                .exclude(
                    participant_submitter=self.object,
                )
                .prefetch_related(
        ...
        if not self.debate.adjudicators.has_chair:
            return self.error_page(
                _(
                    "Your debate doesn't have a chair, so you can't enter results for it. "
                    "Please contact a tab room official."
                )
            )
        ...
        if (
            ...
            return self.error_page(
                _(
                    "It looks like the sides for this debate haven't yet been confirmed, "
    ...
    def postprocess_result(self):
        if self.ballotsub.single_adj and self.tournament.pref("disable_ballot_confirms"):
            ...
            bses = (
                ...
                .select_related("participant_submitter")
                .annotate(
                    ordering=Window(
                        Rank(), partition_by="participant_submitter", order_by="-version"
                    )
                )
                .filter(ordering=1)
            )
            if (
                len(bses)
                != DebateAdjudicator.objects.filter(debate=self.debate)
                .exclude(type=DebateAdjudicator.TYPE_TRAINEE)
                .count()
            ):
                return
            ...
            if len(errors) == 0:
                ...
                self.debate.result_status = (
                    Debate.STATUS_POSTPONED if has_errors else Debate.STATUS_CONFIRMED
                )
                self.debate.save()
...
class OldPublicNewBallotSetByRandomisedUrlView(
    SingleObjectByRandomisedUrlMixin, BasePublicNewBallotSetView
):
    model = Adjudicator
    allow_null_tournament = True
    private_url = True
    ...
    def get_success_url(self):
        return reverse_tournament(
            "privateurls-person-index", self.tournament, kwargs={"url_key": self.kwargs["url_key"]}
        )
...
class BasePublicBallotScoresheetsView(
    PublicTournamentPageMixin, SingleObjectFromTournamentMixin, TemplateView
):
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .exclude(debateteam__side=DebateTeam.Side.BYE)
            .select_related(
                "round",
            )
...
class PublicBallotScoresheetsView(BasePublicBallotScoresheetsView):
    ...
    def check_permissions(self):
        ...
        if round.silent and not round.tournament.pref("all_results_released"):
            logger.warning(
                "Refused public view of ballots for %s: %s is silent", debate, round.name
            )
        ...
        if not round.completed and not round.tournament.pref("all_results_released"):
            logger.warning(
                "Refused public view of ballots for %s: %s is not completed", debate, round.name
            )
            return (
                403,
                _("This debate is in %s, the results for which aren't available yet.") % round.name,
            )
        ...
        if debate.confirmed_ballot is None:
            ...
            return (
                404,
                _("The debate %s does not have a confirmed ballot.") % self.matchup_description(),
            )

    def get_context_data(self, **kwargs):
        kwargs["motion"] = (
            self.object.confirmed_ballot.motion or self.object.round.motion_set.first()
        )
...
class AdjudicatorPrivateUrlBallotScoresheetView(
    RoundMixin, SingleObjectByRandomisedUrlMixin, BasePublicBallotScoresheetsView
):
    ...
    def response_error(self, error):
        ...
        return self.response_class(
            ...
            context={
                "message": message,
                "adjudicator": Adjudicator.objects.get(url_key=self.kwargs.get("url_key")),
            },
            using=self.template_engine,
            status=status,
        )
...
class SpeakerPrivateUrlBallotScoresheetView(
    RoundMixin, SingleObjectByRandomisedUrlMixin, PublicBallotScoresheetsView
):
...
class PublicBallotSubmissionIndexView(PublicTournamentPageMixin, RoundMixin, VueTableTemplateView):
    ...
    def is_draw_released(self):
        return (
            self.round.draw_status == Round.Status.RELEASED and self.round.motions_good_for_public
        )
    ...
    def get_table(self):
        ...
        data = [
            {
                "text": _("Add result from %(adjudicator)s")
                % {"adjudicator": escape(da.adjudicator.get_public_name(self.tournament))},
                "link": reverse_round(
                    "old-results-public-ballotset-new-pk",
                    self.round,
