...
class Command(TournamentCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-c", "--compare", type=str, required=True, help="Tournament to compare to"
        )
    ...
    def handle_tournament(self, tournament, **options):
        ...
        debates = Debate.objects.filter(
            round__tournament=tournament, round__stage=Round.Stage.PRELIMINARY
        )
        ...
        for debate in debates:
            ...
            if original.motion != compare.motion:
                self.stdout.write(
                    "{debate}: original motion={orig}, check motion={check}".format(
                        debate=debate,
                        orig=original.motion.reference,
                        check=compare.motion.reference,
                    )
                )
            ...
            for ts in original.teamscore_set.all():
                ...
                for field in ["points", "win", "margin", "score", "votes_given", "votes_possible"]:
                    if getattr(ts, field) != getattr(cts, field):
                        self.stdout.write(
                            "{dt}: original {field}={orig}, check {field}={check}".format(
                                dt=ts.debate_team,
                                orig=getattr(ts, field),
                                check=getattr(cts, field),
                                field=field,
                            )
                        )

            for ss in original.speakerscore_set.all():
                css = compare.speakerscore_set.get(
                    debate_team__side=ss.debate_team.side, position=ss.position
                )
                for field in ["speaker", "score", "ghost"]:
                    if getattr(ss, field) != getattr(css, field):
                        message = "{dt}, speaker {pos}: original {field}={orig}, check {field}={check}".format(
                            dt=ss.debate_team,
                            pos=ss.position,
                            orig=getattr(ss, field),
                            check=getattr(css, field),
                            field=field,
                        )
                        if field == "speaker":
            ...
            for ssba in original.speakerscorebyadj_set.all():
                ...
                if ssba.score != cssba.score:
                    self.stdout.write(
                        "{dt}, speaker {pos}, from {adj}: original score={orig}, check score={check}".format(
                            dt=ssba.debate_team,
                            pos=ssba.position,
                            adj=ssba.debate_adjudicator.adjudicator,
                            orig=ssba.score,
                            check=cssba.score,
                        )
                    )
        ...
        if no_original:
            self.stdout.write(
                "WARNING: original ballots for {:d} debates not found".format(no_original)
            )
        if no_check:
