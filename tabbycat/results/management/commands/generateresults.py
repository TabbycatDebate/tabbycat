...
from results.dbutils import (
    add_result,
    add_results_to_round,
    add_results_to_round_partial,
    delete_all_ballotsubs_for_round,
    delete_ballotsub,
)
...
class GenerateResultsCommandMixin:
    ...
    def add_arguments(self, parser):
        ...
        results_group.add_argument(
            "-u", "--user", type=str, default="random", help="Username of submitter"
        )
        results_group.add_argument(
            "--create-user", action="store_true", help="Create user if it doesn't exist"
        )
        results_group.add_argument(
            "--reply-random",
            action="store_true",
        ...
        status.add_argument(
            "-D", "--discarded", action="store_true", help="Make added ballot sets discarded"
        )
        status.add_argument(
            "-d",
            "--draft",
        ...
        status.add_argument(
            "-c", "--confirmed", action="store_true", help="Make added ballot sets confirmed"
        )
    ...
    def _get_user(self, options):
        try:
            ...
            if options["create_user"]:
                self.stdout.write(self.style.MIGRATE_HEADING(f"Creating user: {options['user']}"))
                return User.objects.create_user(options["user"], "", options["user"])
            else:
                raise CommandError(
                    "There is no user called {user!r}. Use the --create-user option to create it.".format(
                        user=options["user"]
                    )
                )
...
class Command(GenerateResultsCommandMixin, RoundCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--clean", help="Remove all associated ballot sets first", action="store_true"
        )
        parser.add_argument(
            "-n",
            "--num-ballots",
            type=int,
            help="Number of ballot sets to add per round " "(default all) or debate (default 1)",
            default=None,
        )
    ...
    def handle_round(self, round, **options):
        if options["clean"]:
            self.stdout.write(
                self.style.WARNING("Deleting all ballot sets for {}...".format(round.name))
            )
        ...
        try:
            if options["num_ballots"] is not None:
                ...
                add_results_to_round_partial(
                    round, options["num_ballots"], **self.result_kwargs(options)
                )
            else:
                self.stdout.write(
                    self.style.MIGRATE_HEADING(
    ...
    def handle_debate(self, debate, **options):
        if options["clean"]:
            self.stdout.write(
                self.style.WARNING(
                    "Deleting all ballot sets for debate {}...".format(debate.matchup)
                )
            )
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                "Generating ballot set for debate {}...".format(debate.matchup)
            )
        )
        try:
