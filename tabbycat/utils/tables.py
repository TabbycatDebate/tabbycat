...
class BaseTableBuilder:
    ...
    def _convert_header(header):
        if isinstance(header, dict):
            ...
            warnings.warn(
                "Plain-text headers are deprecated, use a dict with key and title instead",
                stacklevel=3,
            )
    ...
    def add_column(self, header, data):
        ...
        if 0 < len(self.data) != len(data):
            raise ValueError(
                "data contains {new:d} rows, existing table has {existing:d}".format(
...
class TabbycatTableBuilder(BaseTableBuilder):
    ...
    def _adjudicator_record_link(self, adj, suffix=""):
        ...
        if self.admin:
            return {
                ...
                "link": reverse_tournament(
                    "participants-adjudicator-record", self.tournament, kwargs={"pk": adj.pk}
                ),
            }
            ...
            return {
                "text": _("View %(a)s's %(d)s Record") % {"a": escape(adj_short_name), "d": suffix},
                "link": reverse_tournament(
                    "participants-public-adjudicator-record", self.tournament, kwargs={"pk": adj.pk}
                ),
            }
        else:
    ...
    def _team_record_link(self, team):
        if self.admin:
            return {
                ...
                "link": reverse_tournament(
                    "participants-team-record", self.tournament, kwargs={"pk": team.pk}
                ),
            }
        elif self.tournament.pref("public_record"):
            return {
                "text": _("View %(team)s's Record") % {"team": self._team_short_name(team)},
                "link": reverse_tournament(
                    "participants-public-team-record", self.tournament, kwargs={"pk": team.pk}
                ),
            }
        else:
            return {"text": "", "link": False}
    ...
    def _team_cell(self, team, show_emoji=False, subtext=None, highlight=False):
        cell = {
            ...
            "emoji": (
                escape(team.emoji)
                if team.emoji and show_emoji and self.tournament.pref("show_emoji")
                else None
            ),
        ...
        if self.tournament.pref("team_code_names") == "all-tooltips" or (  # code names in tooltips
            ...
            cell["popover"]["content"].append(
                {
                    "text": _("Code name: <strong>%(name)s</strong>")
                    % {"name": escape(team.code_name)}
                }
            )
        if self.tournament.pref("team_code_names") == "admin-tooltips-real" and self.admin:
            cell["popover"]["content"].append(
                {
                    "text": _("Real name: <strong>%(name)s</strong>")
        ...
        if self._show_speakers_in_draw:
            if self.admin and has_permission(self.user, Permission.VIEW_ANONYMOUS, self.tournament):
                speakers = [
                    (
                        "<span class='admin-redacted'>%s</span>" % escape(s.name)
                        if s.anonymous
                        else escape(s.name)
                    )
                ...
                speakers = [
                    (
                        self.REDACTED_CELL["text"]
                        if s.anonymous
                        else escape(s.get_public_name(self.tournament))
                    )
                    for s in team.speakers
                ]
    ...
    BP_POINT_ICONCLASSES = (
        "text-danger result-icon",
        "text-warning result-icon",
        "text-info result-icon",
        "text-success result-icon",
    )
    ...
    def _result_cell_class_four(self, points, cell):
        ...
        cell["popover"]["title"] = _("%(team)s placed %(place)s") % {
            "team": team_name,
            "place": ordinal(4 - points),
        }
            ...
            cell["popover"]["content"].append(
                {
                    ...
                    "link": reverse_tournament(
                        link, self.tournament, kwargs={"pk": ts.ballot_submission_id}
                    ),
                }
            )
            ...
            cell["popover"]["content"].append(
                {
                    ...
                    "link": reverse_tournament(
                        "results-public-scoresheet-view",
                        self.tournament,
    ...
    def _result_cell_two(self, ts, compress=False, show_score=False, show_ballots=False):
        ...
        cell = {
            ...
            % {"opposition": opp_vshort if compress else self._team_short_name(opp)},
        ...
        if ts.win is True:
            ...
            cell["popover"]["title"] = _("No result for debate against %(team)s") % {
                "team": self._team_long_name(opp)
            }
        ...
        if self._show_speakers_in_draw:
            cell["popover"]["content"].append(
                {
                    "text": ", ".join(
                        [escape(s.get_public_name(self.tournament)) for s in opp.speakers]
                    ),
                }
            )
    ...
    def add_tournament_column(self, tournaments):
        header = {
            "key": "tournament",
            "icon": "tag",
            "tooltip": _("Tournament"),
        }
        data = [
            {
                "sort": t.seq,
                "text": escape(t.short_name),
                "tooltip": escape(t.short_name),
            }
    ...
    def add_round_column(self, rounds):
        ...
        data = [
            {
                "sort": round.seq,
                "text": escape(round.abbreviation),
                "tooltip": escape(round.name),
            }
            for round in rounds
        ]
        self.add_column(header, data)

    def add_adjudicator_columns(
        self, adjudicators, show_institutions=True, show_metadata=True, subtext=None
    ):

        adj_data = []
        for adj in adjudicators:
            if adj.anonymous and not (
                self.admin and has_permission(self.user, Permission.VIEW_ANONYMOUS, self.tournament)
            ):
                ...
                if self._show_record_links:
                    cell["popover"] = {
                        "title": escape(adj.get_public_name(self.tournament)),
                        "content": [self._adjudicator_record_link(adj)],
                    }
        ...
        if show_institutions and self.tournament.pref("show_adjudicator_institutions"):
            self.add_column(
                ...
                [
                    escape(adj.institution.code) if adj.institution else self.BLANK_TEXT
                    for adj in adjudicators
                ],
            )
    ...
    def add_debate_adjudicators_column(
        self, debates, title="Adjudicators", show_splits=False, highlight_adj=None, for_admin=False
    ):
        da_data = []
        ...
        def construct_text(adjs_data):
            adjs_list = []
            for a in adjs_data:
                adj_str = '<span class="d-inline">' + escape(
                    a["adj"].get_public_name(self.tournament)
                )
            ...
            return (
                ...
                "</div><span class='d-none d-md-inline'>, </span>"
            ).join(adjs_list)
        ...
        def construct_popover(adjs_data):
            ...
            for a in adjs_data:
                ...
                if (for_admin or self.tournament.pref("show_adjudicator_institutions")) and a[
                    "adj"
        ...
        for debate in debates:
            ...
            if (
                show_splits
                and self.tournament.ballots_per_debate(debate.round.stage) == "per-adj"
                ...
                for (
                    ...
                    position,
                    split,
                ) in debate.confirmed_ballot.result.adjudicators_with_splits():
    ...
    def add_debate_motion_column(self, debates):
        ...
        motions = []
        for debate in debates:
            released = debate.round.motions_released or debate.round.tournament.pref(
                "all_results_released"
            )
    ...
    def add_team_columns(self, teams, show_break_categories=False, show_emoji=True, key=None):
        ...
        team_data = [
            (
                self._team_cell(team, show_emoji=show_emoji)
        ...
        if (
            show_break_categories
            and self.tournament.breakcategory_set.filter(is_general=False).exists()
        ):
            self.add_column(
                {"key": "categories", "icon": "user-check", "tooltip": _("Categories")},
                [
                    ", ".join(
                        escape(bc.name) for bc in getattr(team, "break_categories_nongeneral", [])
                    )
                    for team in teams
                ],
            )
        ...
        for speaker in speakers:
            ...
            if anonymous and not (
                ...
                cell = {
                    ...
                    "class": (
                        "no-wrap" if len(speaker.get_public_name(self.tournament)) < 20 else ""
                    ),
                }
                if anonymous:
                    cell["class"] += " admin-redacted"
    ...
    def add_debate_venue_columns(self, debates, with_times=True, for_admin=False):
        ...
        def construct_venue_cell(venue):
            ...
            cell = {
                "text": escape(venue.display_name),
                "class": "venue-name",
                "link": escape(venue.url),
            }
            ...
            if len(descriptions) > 0:
                if len(descriptions) == 1:
                    categories_sentence = _("This room %(predicate)s.") % {
                        "predicate": descriptions[0]
                    }
                else:
        ...
        for debate in debates:
            ...
            if not debate.is_bye:
                conflicts += [
                    ...
                    for side in self.tournament.sides
                    for flag in debate.get_dt(side).flags
                ]
                ...
                if history > 0:
                    conflicts.append(
                        (
                            "warning",
                            ngettext(
                                "Teams have met once", "Teams have met %(count)d times", history
                            )
                            % {"count": history},
                        )
                    )
            ...
            if self.tournament.pref("avoid_same_institution"):
                institutions = [
                    t.institution_id for t in debate.teams if t.institution_id is not None
                ]
        ...
        conflicts_data = [
            {
                "text": "".join(
                    [
                        '<div class="text-{0}">{1}</div>'.format(*conflict)
    ...
    def _standings_headers(self, info_list):
        ...
        for info in info_list:
            header = {"key": info["abbr"], "tooltip": force_str(info["name"]).capitalize()}
            if info["icon"]:
    ...
    def add_metric_columns(self, standings, integer_score_columns=[]):
        ...
        for standing in standings:
            ...
            for key, metric in zip(standings.metric_keys, standing.itermetrics()):
                ...
                if (
                    key in integer_score_columns
                    and hasattr(metric, "is_integer")
                    and metric.is_integer()
                ):
                    metric = int(metric)
                try:
                    sort = float(metric)
    ...
    def add_speaker_debate_ballot_link_column(self, debates):
        ballot_links_header = {
            "key": "ballot",
            "icon": "search",
            "tooltip": _("The confirmed ballot"),
        }
        ballot_links_data = []
                ...
                ballot_links_data.append(
                    {
                        "text": _("View Ballot"),
                        "link": reverse_round(
                            "speaker-results-privateurl-scoresheet",
                            debate.round,
                            kwargs={"url_key": self.private_url_key},
                        ),
                    }
                )
    ...
    def add_debate_ballot_link_column(self, debates, show_ballot=False):
        ...
        if self.admin:
                ...
                (
                    {
                        ...
                        "link": (
                            None
                            if debate.is_bye
                            else reverse_tournament(
                                "old-results-ballotset-edit",
                                self.tournament,
                                kwargs={"pk": debate.confirmed_ballot.id},
                            )
                        ),
                    }
            ...
            debateqs = (
                Debate.objects.filter(pk__in=[d.pk for d in debates])
                .select_related("round")
                .annotate(
                    has_ballot=Exists(
                        BallotSubmission.objects.filter(debate_id=OuterRef("id")).exclude(
                            discarded=True
                        )
                    ),
                )
            ...
            for o_debate in debates:
                ...
                elif not get_result_class(
                    debate.nondiscard_ballots[0], debate.round, self.tournament
                ).uses_speakers:
                    ballot_links_data.append(_("No scores"))
                else:
                    ballot_links_data.append(
                        {
                            ...
                            "link": (
                                ...
                                if debate.is_bye
                                    ...
                                    "results-privateurl-scoresheet-view",
                                    debate.round,
                                    kwargs={"url_key": self.private_url_key},
                                )
                            ),
    ...
    def add_team_results_columns(self, teams, rounds):
        ...
        for round_seq, round in enumerate(rounds):
            results = [self._result_cell(t.round_results[round_seq]) for t in teams]
    ...
    def add_debate_results_columns(self, debates, iron=False):
        all_sides_confirmed = all(
            debate.sides_confirmed for debate in debates
        )  # should already be fetched
        side_abbrs = {
            side: get_side_name(self.tournament, side, "abbr") for side in self.tournament.sides
        }
        ...
        for debate in debates:
            ...
            for side in self.tournament.sides:
                ...
                subtext = (
                    None
                    if (all_sides_confirmed or not debate.sides_confirmed)
                    else side_abbrs[side]
                )
                ...
                if iron and (debateteam.iron > 0 or debateteam.iron_prev > 0):
                    ...
                    cell["popover"]["content"].append(
                        {"text": "<span class='%s'>%s</span>" % (warning_level, popover_text)}
                    )
    ...
    def add_debate_postponement_column(self, debates):
        col_data = [
            render_to_string("debate_postponement_form.html", {"debate": d}) for d in debates
        ]
