...
class TestTrivialStandings(TestCase):
    ...
    def setUp(self):
        self.tournament = Tournament.objects.create(
            slug="trivialstandingstest", name="Trivial standings test"
        )
        ...
        for i in [1, 2]:
            ...
            dt1 = DebateTeam.objects.create(
                debate=debate, team=self.team1, side=DebateTeam.Side.AFF, flags=["pullup"]
            )
            dt2 = DebateTeam.objects.create(
                debate=debate, team=self.team2, side=DebateTeam.Side.NEG
            )
    ...
    def test_speaks_stddev(self):
        self._base_metric_test({"speaks_stddev": [0.5, 0.5]})
    ...
    def test_double_metric_error(self):
        self.assertRaises(
            StandingsError, TeamStandingsGenerator, ("points", "wbw", "points"), ("rank",)
        )

    def test_points_with_extra_team(self):
        # check that a team with no debates doesn't throw off the rankings
        team_extra = Team.objects.create(
            tournament=self.tournament, reference="extra", use_institution_prefix=False
        )
        generator = TeamStandingsGenerator(("points",), ("rank",))
        standings = self.get_standings(generator)
        self.assertEqual(standings.get_standing(self.team1).rankings["rank"], (1, False))
...
class TestBasicStandings(TestCase):
    ...
    def setup_testdata(self, testdata):
        tournament = Tournament.objects.create(
            slug="basicstandingstest", name="Basic standings test"
        )
        teams = dict()
        ...
        for r, debatedict in enumerate(testdata["teamscores"]):
            ...
            for adj, venue, (teamnames, teamscores) in zip(adjs, venues, debatedict.items()):
                ...
                for team, teamscore_dict in teamscores.items():
                    ...
                    TeamScore.objects.create(
                        debate_team=dt, ballot_submission=ballotsub, **teamscore_dict
                    )
        ...
        for index, testdata in self.testdata.items():
            ...
            for metrics in testdata["rankings"].keys():
                with self.subTest(index=index, metrics=metrics):
                    ...
                    with (
                        suppress_logs("standings.teams", logging.INFO),
...
class TestMissingStandings(TestCase):
    ...
    def setUp(self):
        self.tournament = Tournament.objects.create(
            slug="missingstandingstest", name="Missing standings test"
        )
        self.team1 = Team.objects.create(
            tournament=self.tournament, reference="1", use_institution_prefix=False
        )
