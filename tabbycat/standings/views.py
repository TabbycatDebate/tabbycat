...
from .round_results import (
    add_speaker_round_results,
    add_team_round_results,
    add_team_round_results_public,
)
...
class StandingsIndexView(AdministratorMixin, RoundMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["round_speaks"] = [{"round": "Overall (for in-rounds)", "score": overall}]
        for r in self.tournament.round_set.order_by("seq"):
            avg = speaks.filter(debate_team__debate__round=r).aggregate(Avg("score"))["score__avg"]
            if avg:
...
class BaseStandingsView(RoundMixin, VueTableTemplateView):
    ...
    standings_error_message = gettext_lazy(
        "<p>There was an error generating the standings: " "<em>%(message)s</em></p>",
    )
...
class PublicTabMixin(PublicTournamentPageMixin):
    ...
    def round(self):
        ...
        if self._round.silent and not self.tournament.pref("all_results_released"):
            self._round = (
                self.tournament.prelim_rounds(until=self._round)
                .filter(silent=False)
                .order_by("seq")
                .last()
            )
...
class BaseSpeakerStandingsView(BaseStandingsView):
    ...
    def get_standings(self):
        if self.round is None:
            raise StandingsError(
                _(
                    "The tab can't be displayed because all rounds so far in this tournament are silent."
                )
            )
        ...
        generator = SpeakerStandingsGenerator(
            metrics, self.rankings, extra_metrics, rank_filter=rank_filter
        )
    ...
    def get_table(self):
        ...
        scores_headers = [
            {"key": escape(round.abbreviation), "title": escape(round.abbreviation)}
            for round in rounds
        ]
        scores_data = [
            [metricformat(x) if x is not None else "—" for x in standing.scores]
            for standing in standings
        ]
        table.add_columns(scores_headers, scores_data)
        table.add_metric_columns(
            standings, integer_score_columns=self.integer_score_columns(rounds)
        )
    ...
    def get_rank_filter(self):
        missable = (
            -1
            if self.missable_preference is None
            else self.tournament.pref(self.missable_preference)
        )
        if missable < 0:
            return None, None  # no limit
        total_prelim_rounds = self.tournament.round_set.filter(
            stage=Round.Stage.PRELIMINARY, seq__lte=self.round.seq
        ).count()
    ...
    def cast_round_results(self, standings, rounds, step_preference):
        ...
        if self.tournament.pref(step_preference) % 1 == 0:
            is_consensus_by_round = [
                self.tournament.ballots_per_debate(r.stage) == "per-debate" for r in rounds
            ]
            for info in standings:
...
class BaseSubstantiveSpeakerStandingsView(BaseSpeakerStandingsView):
    ...
    def get_metrics(self):
        ...
        if (
            self.tournament.pref("standings_missed_debates") >= 0
            and "count" not in metrics
            and "count" not in extra_metrics
        ):
...
class BaseSpeakerCategoryStandingsView(
    SingleObjectFromTournamentMixin, BaseSubstantiveSpeakerStandingsView
):
...
class PublicSpeakerCategoryTabView(PublicTabMixin, BaseSpeakerCategoryStandingsView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not self.object.public:
            logger.warning(
                "Tried to access a non-public speaker category tab page: %s", self.object.slug
            )
...
class BaseTeamStandingsView(BaseStandingsView):
    ...
    def get_standings(self):
        ...
        teams = teams.select_related("institution").prefetch_related(
            "speaker_set",
            Prefetch(
                "break_categories",
    ...
    def get_rounds(self):
        if not hasattr(self, "_rounds"):
            if self.tournament.pref("all_results_released"):
                ...
                self._rounds = (
                    self.tournament.prelim_rounds(before=self.tournament.current_round)
                    .filter(silent=False)
                    .order_by("seq")
                )
        return self._rounds
...
class PublicAdjudicatorsTabView(PublicTabMixin, BaseFeedbackOverview):
    ...
    def annotate_table(self, table, adjudicators):
        ...
        if (
            self.tournament.pref("adjudicators_tab_shows") == "final"
            or self.tournament.pref("adjudicators_tab_shows") == "all"
        ):
        ...
        if (
            self.tournament.pref("adjudicators_tab_shows") == "test"
            or self.tournament.pref("adjudicators_tab_shows") == "all"
        ):
        ...
        messages.info(
            self.request,
            _(
                "An adjudicator's score is determined by "
...
class EmailTeamStandingsView(RoundTemplateEmailCreateView):
    ...
    def get_default_send_queryset(self):
        return Speaker.objects.filter(
            team__round_availabilities__round=self.round, email__isnull=False
    ...
    def get_extra(self):
        ...
        if self.tournament.pref("public_team_standings"):
            extra["url"] = self.request.build_absolute_uri(
                reverse_tournament("standings-public-teams-current", self.tournament)
            )
        else:
            extra["url"] = ""
        return extra
