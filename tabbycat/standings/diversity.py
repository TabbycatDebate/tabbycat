...
def _gender_group(gender_field):
    return Case(
        When(
            **{
                "%s__in" % gender_field: (Person.GENDER_FEMALE, Person.GENDER_OTHER),
                "then": Value("N"),
            }
        ),
...
def compile_statistics_by_gender(titles, queryset, statistics, gender_field):
    ...
    for title, statistic in zip(titles, statistics):
        ...
        result["data"] = _group_data(
            lambda gender: gender_statistics[gender][statistic], ["N", "M"], ["NM", "Male"]
        )
...
def compile_grouped_means_by_gender(titles, queryset, gender_field, group_field, group_values):
    ...
    gender_means = queryset.values(group_field, gender=_gender_group(gender_field)).annotate(
        Avg("score")
    )
    ...
    for title, group in zip(titles, group_values):
        ...
        result["data"] = _group_data(
            lambda gender: gender_means[(group, gender)], ["N", "M"], ["NM", "Male"]
        )
        results.append(result)
...
def compile_gender_counts(title, queryset, gender_field):
    return compile_grouped_counts(
        title, queryset, _gender_group(gender_field), ["N", "M", "-"], ["NM", "Male", "Unknown"]
    )


def compile_grouped_counts(title, queryset, group_field, group_values, group_labels):
    counts = queryset.values(group=group_field).annotate(
        count=Count(Value(1))
    )  # Count counts records, value doesn't matter
...
def compile_grouped_gender_counts(titles, queryset, gender_field, group_field, group_values):
    counts = queryset.values(group_field, gender_group=_gender_group(gender_field)).annotate(
        count=Count(Value(1))
    )
    ...
    for title, group in zip(titles, group_values):
        result = {"title": title}
        result["data"] = _group_data(
            lambda gender: counts[(group, gender)], ["N", "M", "-"], ["NM", "Male", "Unknown"]
        )
        results.append(result)
    return results
...
def get_diversity_data_sets(t, for_public):
    ...
    if t.pref("public_breaking_teams") is True or for_public is False:
        if (
            ...
            .filter(team__breakingteam__isnull=False)
            .count()
            > 0
        ):
            data_sets["speakers_gender"].append(
                compile_gender_counts(
                    _("Breaking"), speakers.filter(team__breakingteam__isnull=False), "gender"
                )
            )

    for sc in SpeakerCategory.objects.filter(tournament=t).order_by("seq"):
        if Speaker.objects.filter(categories=sc).count() > 0:
            data_sets["speakers_categories"].append(
                compile_gender_counts(sc.name, speakers.filter(categories=sc), "gender")
            )
            data_sets["speakers_categories"].append(
                compile_gender_counts(
                    _("Not %(category)s") % {"category": sc.name},
                    speakers.exclude(categories=sc),
                    "gender",
                )
            )
    ...
    if Team.objects.exclude(institution__region__isnull=True).exists():
        data_sets["speakers_region"].append(
            compile_grouped_counts(
                _("All Speakers"),
                speakers,
                F("team__institution__region__id"),
                region_values,
                region_labels,
            )
        )
    ...
    if adjudicators.count() > 0:
        data_sets["adjudicators_gender"].append(
            compile_gender_counts(_("All"), adjudicators, "gender")
        )

    if Adjudicator.objects.filter(tournament=t).filter(independent=True).exists():
        data_sets["adjudicators_gender"].append(
            compile_gender_counts(_("IAs"), adjudicators.filter(independent=True), "gender")
        )

    if (
        t.pref("public_breaking_adjs") is True or for_public is False
    ) and Adjudicator.objects.filter(breaking=True).exists():
        data_sets["adjudicators_gender"].append(
            compile_gender_counts(_("Breaking"), adjudicators.filter(breaking=True), "gender")
        )
    ...
    data_sets["adjudicators_position"] = compile_grouped_gender_counts(
        titles, debateadjs, "adjudicator__gender", "type", adjtypes
    )

    if Adjudicator.objects.exclude(institution__region__isnull=True).exists():
        data_sets["adjudicators_region"].append(
            compile_grouped_counts(
                _("All"), adjudicators, F("institution__region__id"), region_values, region_labels
            )
        )
        ...
        if t.pref("public_breaking_adjs") is True or for_public is False:
            data_sets["adjudicators_region"].append(
                compile_grouped_counts(
                    _("Breaking"),
                    adjudicators.filter(breaking=True),
                    F("institution__region__id"),
                    region_values,
                    region_labels,
                )
            )
    ...
    data_sets["gendered_adjudicators"] = (
        Adjudicator.objects.filter(gender="M").count()
        + Adjudicator.objects.filter(gender="F").count()
    )
    if data_sets["gendered_adjudicators"] > 0:
        ...
        if data_sets["feedbacks_count"] > 0:
            ...
            data_sets["adjudicators_results"] = compile_statistics_by_gender(
                titles, adjfeedbacks, statistics, "adjudicator__gender"
            )
    ...
    # Don't show data if genders have not been set
    data_sets["gendered_speakers"] = (
        Speaker.objects.filter(gender="M").count() + Speaker.objects.filter(gender="F").count()
    )
    if data_sets["gendered_speakers"] > 0:

        speakerscores = SpeakerScore.objects.filter(
            speaker__team__tournament=t, ballot_submission__confirmed=True
        )
        ...
        if data_sets["speaks_count"] > 0:
            ...
            statistics = ["mean", "median", "upperq", "lowerq"]
            data_sets["speakers_results"] = compile_statistics_by_gender(
                titles,
                speakerscores.exclude(position=t.reply_position),
                statistics,
                "speaker__gender",
            )
            ...
            titles = [
                (
                    _("Reply Speaker Average")
                    if pos == t.reply_position
                    else _("Speaker %(num)d Average") % {"num": pos}
                )
            ...
            if speakerscores.filter(
                debate_team__debate__round__stage=Round.Stage.ELIMINATION
            ).exists():
                data_sets["detailed_speakers_results"].extend(
                    compile_statistics_by_gender(
                        [_("Average Finals Score")],
                        speakerscores.filter(
                            debate_team__debate__round__stage=Round.Stage.ELIMINATION
                        ).exclude(position=t.reply_position),
                        ["mean"],
                        "speaker__gender",
                    )
                )
