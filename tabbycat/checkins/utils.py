...
def single_checkin(instance, events):
    ...
    try:
        ...
        instance.checked_tooltip = _("Not checked in (barcode %(barcode)s)") % {
    ...
    if identifier:
        instance.time = next(
            (e["time"] for e in events if e["identifier__barcode"] == identifier.barcode), None
        )
        if instance.time:
            instance.checked_in = True
            instance.checked_icon = "check"
            instance.checked_tooltip = _("checked in at %(time)s") % {
...
def multi_checkin(team, events, t):
    ...
    for speaker in team.speaker_set.all():
        ...
        if speaker.checked_in:
            tooltip = _("%(speaker)s checked in at %(time)s.") % {
                "speaker": speaker.get_public_name(t),
                "time": speaker.time.strftime("%H:%M"),
            }
        else:
...
def get_checkins(queryset, t, window_preference_type):
    events = get_unexpired_checkins(t, window_preference_type).values("time", "identifier__barcode")
