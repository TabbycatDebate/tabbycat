...
class Command(TournamentCommand):
    ...
    def handle_tournament(self, tournament, **options):
        ...
        if (
            ...
            or options["subcommand"] == "generate"
            and options["overwrite"]
        ):
            _, counts = delete_identifiers(queryset)
            self.stdout.write(
                "Deleted check-in identifiers for all {0:d} {1:s}".format(existing_count, plural)
            )
        ...
        if options["subcommand"] == "generate":
            ...
            self.stdout.write(
                "Generated check-in identifiers for {0:d} {1:s}".format(queryset.count(), plural)
            )
