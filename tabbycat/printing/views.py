...
class BasePrintFeedbackFormsView(RoundMixin, TemplateView):
    ...
    def construct_info(self, venue, source, source_p, target, target_p):
        ...
        return {
            ...
            "authorInstitution": (
                escape(source.institution.code) if source.institution else _("Unaffiliated")
            ),
            "author": escape(source_n),
            "authorPosition": source_p,
            "target": escape(target.name),
            "targetPosition": target_p,
        }
    ...
    def get_team_feedbacks(self, debate, team):
        ...
        if team_paths == "orallist" and debate.adjudicators.chair:
            ballots.append(
                self.construct_info(debate.venue, team, _("Team"), debate.adjudicators.chair, "")
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["team_questions_exist"] = self.tournament.adjudicatorfeedbackquestion_set.filter(
            from_team=True
        ).exists()
        kwargs["adj_questions_exist"] = self.tournament.adjudicatorfeedbackquestion_set.filter(
            from_adj=True
        ).exists()
...
class AssistantPrintFeedbackFormsView(
    CurrentRoundMixin, OptionalAssistantTournamentPageMixin, BasePrintFeedbackFormsView
):
...
class BasePrintScoresheetsView(RoundMixin, TemplateView):
    ...
    def get_ballots_dicts(self):
        ...
        for debate in draw:
            ...
            debate_dict["barcode"] = next(
                (i["barcode"] for i in identifiers if i["debate_id"] == debate.id), None
            )
            ...
            for side, (side_name, positions) in zip(self.tournament.sides, sides_and_positions):
                ...
                try:
                    ...
                    dt_dict["team"] = {
                        ...
                        "speakers": [
                            {"name": escape(s.get_public_name(self.tournament))}
            ...
            for adj, pos in debate.adjudicators.with_positions():
                ...
                da_dict["adjudicator"] = {
                    ...
                    "institution": {
                        "code": (
                            escape(adj.institution.code) if adj.institution else _("Unaffiliated")
                        )
                    },
                }
            ...
            for author, pos in authors:
                if author:
                    ballot_dict = {
                        ...
                        "authorInstitution": (
                            escape(author.institution.code)
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["motions"] = json.dumps(
            [{"seq": m.seq, "text": escape(m.motion.text)} for m in motions]
        )
...
class AssistantPrintScoresheetsView(
    CurrentRoundMixin, OptionalAssistantTournamentPageMixin, BasePrintScoresheetsView
):
    ...
    def add_urls(self, participants):
        for participant in participants:
            url = reverse_tournament(
                "privateurls-person-index",
                self.tournament,
                kwargs={"url_key": participant["url_key"]},
            )
...
class PrintableRandomisedURLsForTeams(BasePrintableRandomisedURLs):
    ...
    def get_participants_for_type(self):
        participants = Speaker.objects.filter(
            team__tournament=self.tournament, url_key__isnull=False
        )
        return list(
            participants.select_related("team").values("name", "team__short_name", "url_key")
        )
...
class PrintableRandomisedURLsForAdjudicators(BasePrintableRandomisedURLs):
    ...
    def get_participants_for_type(self):
        ...
        return list(
            participants.select_related("institution").values(
                "name", "institution__code", "url_key"
            )
        )
