...
class Motion(models.Model):
    ...
    rounds = models.ManyToManyField(
        "tournaments.Round", through="motions.RoundMotion", verbose_name=_("rounds")
    )
...
class DebateTeamMotionPreference(models.Model):
    ...
    motion = models.ForeignKey(Motion, models.CASCADE, db_index=True, verbose_name=_("motion"))
    preference = models.IntegerField(db_index=True, verbose_name=_("preferences"))
    ballot_submission = models.ForeignKey(
        "results.BallotSubmission", models.CASCADE, verbose_name=_("ballot submission")
    )
    ...
    def roundmotion(self):
        if not hasattr(self, "_roundmotion"):
            self._roundmotion = RoundMotion.objects.get(
                motion=self.motion, round_id=self.debate_team.debate.round_id
            )
