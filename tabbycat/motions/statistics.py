...
class MotionTwoTeamStatsCalculator:
    ...
    def __init__(self, tournament):
        ...
        for pk, motion in self.dict_motions.items():
            ...
            motion.χ2_label, motion.χ2_info = self._annotate_χsquared(
                motion.aff_wins, motion.neg_wins
            )

            if self.include_vetoes:
                # vetoes are the "other way round", since an aff veto indicates it's neg-weighted
                motion.veto_χ2_label, motion.veto_χ2_info = self._annotate_χsquared(
                    motion.neg_vetoes, motion.aff_vetoes
                )
    ...
    def _prefetch_motions(self):
        ...
        motions = Motion.objects.filter(
            ...
            ndebates=Count("ballotsubmission", filter=Q(ballotsubmission__confirmed=True)),
        )
        ...
        _annotate_annotations(
            self.dict_motions, motions, ["%s_wins" % side for side in self.tournament.sides]
        )

        if self.include_vetoes:
            motions = Motion.objects.filter(pk__in=self.dict_motions.keys()).annotate(
                **{
                    "%s_vetoes"
                    % side: Count(
            ...
            _annotate_annotations(
                self.dict_motions, motions, ["%s_vetoes" % side for side in self.tournament.sides]
            )
    ...
    def _annotate_χsquared(self, affs, negs):  # noqa: N802
        ...
        for critical, level_str, evidence_str in self.CRITICAL_VALUES:
            if T > critical:
                ...
                info = _(
                    "χ² statistic is %(chisq).3f, providing %(evidence)s to "
                    "suggest that this motion was imbalanced — at a %(level)s level of "
                    "significance."
                ) % {"chisq": T, "level": level_str, "evidence": evidence_str}
                break
        else:
            label = _("probably balanced")
            info = _(
                "χ² statistic is %(chisq).3f, providing insufficient evidence "
                "to suggest that this motion was imbalanced at any level of significance."
            ) % {"chisq": T}
...
class RoundMotionTwoTeamStatsCalculator(MotionTwoTeamStatsCalculator):
    ...
    def _prefetch_motions(self):
        motions = (
            ...
            .prefetch_related("round", "motion")
            .order_by("round__seq", "seq")
        )
        self.dict_motions = {m.id: m for m in motions}

        motions = RoundMotion.objects.filter(pk__in=self.dict_motions.keys()).annotate(
            tdebates=Count("round__debate")
        )
...
class MotionBPStatsCalculator:
    ...
    def __init__(self, tournament):
        ...
        self.motions = itertools.chain(
            self.prelim_motions_dict.values(), self.elim_motions_dict.values()
        )
    ...
    def _prefetch_prelim_motions(self):
        ...
        motions = Motion.objects.filter(pk__in=self.prelim_motions_dict.keys()).annotate(
            **annotations
        )
    ...
    def _prefetch_elim_motions(self):
        ...
        motions = Motion.objects.filter(pk__in=self.elim_motions_dict.keys()).annotate(
            **annotations
        )
    ...
    def _collate_elim_motion_annotations(self):
        ...
        for motion in self.elim_motions_dict.values():
            ...
            for side in self.tournament.sides:
                ...
                motion.counts_by_side.append(
                    (side, advancing, advancing_pc, eliminated, eliminated_pc)
                )
...
class RoundMotionBPStatsCalculator(MotionBPStatsCalculator):
    ...
    def _prefetch_prelim_motions(self):
        ...
        motions = RoundMotion.objects.filter(pk__in=self.prelim_motions_dict.keys()).annotate(
            **annotations
        )
        _annotate_annotations(self.prelim_motions_dict, motions, annotations.keys())
    ...
    def _prefetch_elim_motions(self):
        ...
        motions = RoundMotion.objects.filter(pk__in=self.elim_motions_dict.keys()).annotate(
            **annotations
        )
        _annotate_annotations(self.elim_motions_dict, motions, annotations.keys())
