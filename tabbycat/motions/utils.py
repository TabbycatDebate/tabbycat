...
def merge_motions(new_bs, bses):
    ...
    if n_motions > 1:
        raise ValidationError(
            _(
                "Not all latest ballots have the same motion. The correct motion must be set manually."
            )
        )
    elif n_motions == 1:
...
def merge_motion_vetos(new_bs, bses):
    ...
    pref_lists = [
        tuple(
            bs.debateteammotionpreference_set.all().values_list(
                "debate_team", "debate_team__side", "motion", "preference"
            )
        )
        for bs in bses
    ]
