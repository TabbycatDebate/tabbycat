...
from options.views import (
    SetPresetPreferencesView,
    TournamentConfigIndexView,
    TournamentPreferenceFormView,
)
...
class TournamentPreferenceFormViewTests(TestCase):
    ...
    def test_gets_correct_form(self, mock_builder):
        ...
        view.setup(
            RequestFactory().get(
                reverse_tournament(
                    "options-tournament-section", tournament, kwargs={"section": "scoring"}
                )
            ),
    ...
    def test_section_does_not_exist(self):
        ...
        request = RequestFactory().get(
            reverse_tournament(
                "options-tournament-section", tournament, kwargs={"section": "invalidsection"}
            )
        )
        request.user = get_user_model()(is_superuser=True)
    ...
    def test_save_form(self, mock_redirect, mock_success):
        ...
        form = TestPreset.get_form(
            tournament,
            data={"scoring__score_min": Decimal("70"), "scoring__score_max": Decimal("80")},
        )
        form.is_valid()
...
class TestSetPresetPreferencesView(TestCase):
    ...
    def test_title_with_preset(self, mock_all_presets):
        ...
        request = RequestFactory().get(
            reverse_tournament(
                "options-presets-confirm", tournament, kwargs={"preset_name": "testpreset"}
            )
        )
