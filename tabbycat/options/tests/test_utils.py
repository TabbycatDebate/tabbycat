...
from options.utils import (
    use_team_code_names,
    use_team_code_names_data_entry,
    validate_metric_duplicates,
)
...
class UseTeamCodeNamesTests(TestCase):
    ...
    def test_use_codes_if_setting(self):
        ...
        for setting, admin, user in product(
            [c[0] for c in TeamCodeNames.choices], (False, True), (self.user, AnonymousUser())
        ):
            with self.subTest(setting=setting, admin=admin, user=user.is_anonymous):
                tournament._prefs["team_code_names"] = setting

                if setting in ["admin-tooltips-real", "everywhere"] or (
                    setting == "admin-tooltips-code" and not (admin and not user.is_anonymous)
                ):
    ...
    def test_use_codes_data_entry(self):
        ...
        for setting, tabroom in product([c[0] for c in TeamCodeNames.choices], (False, True)):
            with self.subTest(setting=setting, tabroom=tabroom):
                ...
                elif setting == "everywhere" or (
                    setting in ["admin-tooltips-code", "admin-tooltips-real"] and not tabroom
                ):
...
class DuplicatePreferencesTest(TestCase):
    ...
    def test_unique_metrics(self):
        self.assertIsNone(
            validate_metric_duplicates(TeamStandingsGenerator, ["wins", "speaks_sum"])
        )

    def test_repeatable_metrics(self):
        self.assertIsNone(
            validate_metric_duplicates(TeamStandingsGenerator, ["wins", "wbw", "speaks_sum", "wbw"])
        )
