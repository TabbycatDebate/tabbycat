...
class SpeakerScoreStep(DecimalPreference):
    ...
    help_text = _(
        "Score steps allowed for substantive speeches, e.g. full points (1) or half points (0.5)"
    )
    section = scoring
    name = "score_step"
    default = Decimal("1")
...
class ReplyScoreStep(DecimalPreference):
    help_text = _(
        "Score steps allowed for reply speeches, e.g. full points (1) or half points (0.5)"
    )
...
class AvoidSameInstitution(BooleanPreference):
    help_text = _(
        "If checked, the draw will try to avoid pairing teams against their own institution"
    )
...
class DrawPullupRestriction(ChoicePreference):
    help_text = _(
        "If using pull-ups, restrict which teams can be pulled up. "
...
class BPRenyiOrder(FloatPreference):
    help_text = _(
        "Rényi order α, if BP position cost uses Rényi entropy. "
...
class BPPositionCostExponent(FloatPreference):
    help_text = _(
        "The BP position cost is raised to this power; higher exponents "
...
class BPAssignmentMethod(ChoicePreference):
    help_text = _(
        "In BP, which method to use to solve the assignment problem. "
...
class ByeTeamSelection(ChoicePreference):
    help_text = _(
        "If creating a draw with an uneven number of teams, how to "
...
class FeedbackPaths(ChoicePreference):
    help_text = _(
        "Used to inform available choices in the feedback forms for adjudicators (both online and printed) and feedback progress"
    )
    ...
    choices = (
        ...
        (
            "with-p-on-p",
            _("Panellists on eachother and chairs, trainees on chairs, chairs on everyone"),
        ),
...
class FeedbackFromTeams(ChoicePreference):
    ...
    help_text = _(
        "Used to inform available choices in the feedback forms for teams (both online and printed) and feedback progress; this option is used by, e.g., UADC"
    )
    section = feedback
    ...
    choices = (
        (
            "orallist",
            _("Orallist only (voting panellists permitted, with prompts to select orallist)"),
        ),
...
class ShowUnexpectedFeedback(BooleanPreference):
    ...
    help_text = _(
        "Displays unexpected feedback with a question mark symbol; only relevant if public participants and feedback progress are both enabled"
    )
    section = feedback
...
class BallotsPerDebatePreliminary(ChoicePreference):
    help_text = _(
        "Whether panels submit a ballot each or a single ballot for a debate during the preliminary rounds. Note: BP must use one per debate."
    )
...
class BallotsPerDebateElimination(ChoicePreference):
    help_text = _(
        "Whether panels submit a ballot each or a single ballot for a debate during the elimination rounds. Note: BP must use one per debate."
    )
...
class BallotMustConfirmWinner(ChoicePreference):
    help_text = _(
        "Whether adjudicator(s) must select the winning team in their ballot, and how it should be treated. Note: Not supported in BP."
    )
...
class RequireSubstantiveForReply(BooleanPreference):
    help_text = _(
        "Whether to limit reply speeches to speakers who gave a substantive speech in the debate"
    )
...
class StandingsMissedDebates(IntegerPreference):
    help_text = _(
        "The number of substantive speeches a speaker can miss and still be on the speaker tab (-1 means no limit)"
    )
...
class StandingsMissedReplies(IntegerPreference):
    help_text = _(
        "The number of reply speeches a speaker can miss and still be on the replies tab (-1 means no limit)"
    )
...
class TeamTabReleaseLimit(IntegerPreference):
    help_text = _(
        "Only show scores for the top X teams in the public tab (set to 0 to show all teams)."
    )
...
class SpeakerTabReleased(BooleanPreference):
    help_text = _(
        "Enables public display of the speaker tab. Intended for use after the tournament."
    )
...
class SpeakerTabReleaseLimit(IntegerPreference):
    help_text = _(
        "Only show scores for the top X speakers in the public tab (set to 0 to show all speakers)."
    )
...
class RepliesTabReleased(BooleanPreference):
    help_text = _(
        "Enables public display of the replies tab. Intended for use after the tournament."
    )
...
class RepliesTabReleaseLimit(IntegerPreference):
    help_text = _(
        "Only show scores for the top X repliers in the public tab (set to 0 to show all repliers)."
    )
...
class BallotsReleased(BooleanPreference):
    help_text = _(
        "Enables public display of every adjudicator's ballot. Intended for use after the tournament."
    )
...
class AllResultsReleased(BooleanPreference):
    help_text = _(
        "This releases all the results for all rounds (including silent and break rounds). Do so only after the tournament is finished!"
    )
...
class PrivateBallotsReleased(BooleanPreference):
    help_text = _(
        "Enables display of confirmed ballots through private URLs. Intended for use after the tournament."
    )
...
class PublicUsePassword(BooleanPreference):
    help_text = _(
        "If checked, users must enter a password when submitting public feedback and ballots"
    )
...
class PublicPassword(StringPreference):
    help_text = _(
        "Value of the password required for public submissions, if passwords are required"
    )
...
class EnableBlindBallotConfirmation(BooleanPreference):
    help_text = _(
        "Requires scores of draft ballots to be re-entered as part of the confirmation stage (to create more stringent check). Only applies to BP formats."
    )
...
class AssistantAccess(ChoicePreference):
    help_text = _(
        "Whether assistants can access pages that can reveal matchups "
...
class CheckInWindowPeople(FloatPreference):
    help_text = _(
        "The amount of time (in hours) before a speaker or adjudicator's check-in event expires"
    )
    section = data_entry
...
class BallotsConfirmDigits(BooleanPreference):
    help_text = _(
        "Whether the printed scoresheets should show the 'circle digits' prompt to help check bad handwriting"
    )
...
class ScoreReturnLocation(StringPreference):
    help_text = _(
        "The location to return scoresheets to, printed on pre-printed ballots. Set to 'TBA' to hide."
    )
...
class FeedbackReturnLocation(StringPreference):
    help_text = _(
        "The location to return feedback to, printed on pre-printed feedback forms. Set to 'TBA' to hide."
    )
...
class PublicCheckinStatuses(BooleanPreference):
    help_text = _(
        "Enables the public page showing checkin statuses for "
...
class PublicBreakCategories(BooleanPreference):
    help_text = _(
        "If the participants list is enabled, displays break category eligibility on that page"
    )
...
class PublicMotions(BooleanPreference):
    help_text = _(
        "Enables the public page showing motions that have been explicitly released to the public"
    )
...
class PublicTeamStandings(BooleanPreference):
    help_text = _(
        "Enables the public page showing team standings, showing wins only (not speaker scores or ranking)"
    )
...
class PublicBreakingTeams(BooleanPreference):
    help_text = _(
        "Enables the public page showing the team breaks. Intended for use after the break announcement."
    )
...
class PublicBreakingAdjs(BooleanPreference):
    help_text = _(
        "Enables the public page showing breaking adjudicators. Intended for use after the break announcement."
    )
...
class TournamentStaff(LongStringPreference):
    help_text = _(
        "List of tournament staff, to be displayed on the tournament home page. Leave this blank or with the default text if you want to not show this information."
    )
...
class TeamCodeNames(ChoicePreference):
    ...
    choices = (
        ...
        (
            "admin-tooltips-code",
            _("Use code names for public; real names with code names in " "tooltips for admins"),
        ),
        (
            "admin-tooltips-real",
            _("Use code names for public; code names with real names in " "tooltips for admins"),
        ),
        (
            "everywhere",
            _(
...
class EnableEmailBallotReceipts(BooleanPreference):
    help_text = _(
        "Enables a copy of adjudicators' ballots to be automatically sent to them (by email) after they are entered in Tabbycat (for confirmation or checking)"
    )
...
class BallotEmailSubjectLine(StringPreference):
    help_text = _(
        "The subject line for emails sent to adjudicators with their submitted ballot. "
...
class MotionReleaseEmailMessage(LongStringPreference):
    ...
    default = "<p>The motion(s) for {{ ROUND }} are:</p>" "{{ MOTIONS }}"
...
class BallotsHideMotions(BooleanPreference):
    help_text = _(
        "Whether the printed scoresheets should hide the text of motions (even if they have been entered and released)"
    )
...
class EnableAPIAccess(BooleanPreference):
    help_text = _(
        "Enables external applications to access the site through a dedicated interface, subject to public information settings."
    )
