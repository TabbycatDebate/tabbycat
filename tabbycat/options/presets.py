...
class PreferencesPreset:
    ...
    def get_form(cls, tournament, **kwargs):
        form = tournament_preference_form_builder(
            tournament, [tuple(key.split("__", 1)[::-1]) for key in cls.get_preferences()]
        )(**kwargs)
        for field in form:
...
class AustralsPreferences(PreferencesPreset):
    ...
    description = _(
        "3 vs 3 with replies, chosen motions, intermediate brackets, "
...
class CanadianParliamentaryPreferences(PreferencesPreset):
    ...
    show_in_list = True
    description = _(
        "2 vs 2 with replies (unscored) and POIs. May require "
...
class AustralianEastersPreferences(AustralsPreferences):
    ...
    show_in_list = True
    description = _(
        "3 vs 3 without replies, set motions, novices, intermediate "
...
class NZEastersPreferences(AustralsPreferences):
    ...
    show_in_list = True
    description = _("2 vs 2 with replies, chosen motions, chosen sides, and " "novice statuses.")
    ...
    debate_rules__substantive_speakers = 2
    # Standings
    standings__team_standings_precedence = [
        "wins",
        "wbw",
        "speaks_sum",
        "wbw",
...
class JoyntPreferences(AustralsPreferences):
    ...
    show_in_list = True
    description = _(
        "3 vs 3 with preallocated sides, publicly displayed sides "
...
class UADCPreferences(AustralsPreferences):
    ...
    show_in_list = True
    description = _(
        "3 vs 3 with replies, chosen motions, and all adjudicators "
...
class WSDCPreferences(AustralsPreferences):
    ...
    show_in_list = True
    description = _(
        "3 vs 3 with replies, chosen motions, prop/opp side labels, "
...
class PublicInformation(PreferencesPreset):
    ...
    show_in_list = False
    description = _(
        "For tournaments hosted online: this sets it up so that "
...
class TabRelease(PreferencesPreset):
    ...
    show_in_list = False
    description = _(
        "For when a tab is ready to be released. This will publicly "
