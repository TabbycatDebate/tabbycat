...
import random
testdata = dict()
testdata['scores'] = [[[float(random.randint(70,80)) for pos in xrange(3)] +
        [float(random.randint(70,80))/2] for team in xrange(2)] for adj in xrange(3)]
...
testdata['majority_scores'] = [[sum(adj[team][pos]for adj in majority)/len(majority) for pos in xrange(4)] for team in xrange(2)]
