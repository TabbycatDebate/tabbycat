...
class BaseFeedbackForm(forms.Form):
    ...
    def _create_fields(self):
        ...
        adj_min_score = self.tournament.preferences.get_by_name('adj_min_score')
        adj_max_score = self.tournament.preferences.get_by_name('adj_max_score')
        ...
        if self._use_tournament_password and self.tournament.preferences.get_by_name('public_use_password'):
...
def make_feedback_form_class_for_adj(source, submission_fields, confirm_on_submit=False, enforce_required=True):
    ...
    if not source.tournament.preferences.get_by_name('panellist_feedback_enabled'): # then include only debates for which this adj was the chair
